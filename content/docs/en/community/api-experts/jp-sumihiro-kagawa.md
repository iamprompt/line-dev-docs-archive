---
title: LINE API Expert - Sumihiro Kagawa
navigation: true
description: ''
meta: >-
  {"sidebar":false,"search_index":false,"feedback_form":false,"name":"Sumihiro
  Kagawa","img":"jp-sumihiro-kagawa.png","region":"Japan","location":null,"expert-since":null,"line-api":"LINE
  Front-end Framework, LINE MINI App, LINE Messaging API","skills":"JavaScript,
  TypeScript, Python, IoT, Cloud (AWS,
  GCP)","git":"https://github.com/sumihiro3","fb":"https://www.facebook.com/sumihiro.kagawa","twitter":"https://twitter.com/sumihiro3"}
path: /en/community/api-experts/jp-sumihiro-kagawa
__hash__: kpEdoZ-VFRn89mP0qvmNBHIGXY5SVx77kFiMA-HyOK4
seo:
  title: LINE API Expert - Sumihiro Kagawa
  description: ''
---

::lae-page
#undefined
株式会社ブレイブテクノロジー 取締役 技術本部長

LINEヤフー認定LINEテクノロジーパートナー「LINEミニアプリ部門」である株式会社ブレイブテクノロジーに所属し、開発部門統括(取締役)としてLINEミニアプリを採用した順番待ち管理サービス「matoca」「yoboca」の開発・運用を推進している。

これまでもエンジニア／ITコンサルタントとして民間企業・公共など大小様々なプロジェクトで、テスターからプログラマー、アーキテクト、プロジェクトマネージャー、PMO など幅広く従事している。

#undefined
- [https://sumihiro3.netlify.app/](https://sumihiro3.netlify.app/){rel="[\"nofollow\"]"}
- [https://qiita.com/sumihiro3](https://qiita.com/sumihiro3){rel="[\"nofollow\"]"}
- [https://speakerdeck.com/sumihiro3](https://speakerdeck.com/sumihiro3){rel="[\"nofollow\"]"}

#undefined
::
