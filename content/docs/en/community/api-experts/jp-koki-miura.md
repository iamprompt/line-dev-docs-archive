---
title: LINE API Expert - Koki Miura
navigation: true
description: ''
meta: >-
  {"sidebar":false,"search_index":false,"feedback_form":false,"name":"Koki
  Miura","img":"jp-koki-miura.png","region":"Japan","location":null,"expert-since":null,"line-api":"Messaging
  API","skills":"Python, Node.js, Chatbot, Smart Speaker, Machine Learning, IoT,
  AI, Image Processing,
  Cloud","git":"https://github.com/Miura55","fb":"https://www.facebook.com/koki.miura05","twitter":"https://twitter.com/k_miura_io"}
path: /en/community/api-experts/jp-koki-miura
__hash__: LyA6QzD78uGLxbhHZ8LQ5lfQuv41TEuC1NYRWkpNA74
seo:
  title: LINE API Expert - Koki Miura
  description: ''
---

::lae-page
#undefined
中学時代はFlashを使ってゲームを開発していたが、大学時代にロボット工学を専攻し、ハードよりの開発を始める。しかし、研究室配属をきっかけにプログラミングを始め、その後趣味でLINE bot開発を始めてWeb、クラウドの世界にのめり込み、ハッカソンで成果を残す。2023年8月にAWS Community Builder選出。

#undefined
- [https://zenn.dev/kmiura55](https://zenn.dev/kmiura55){rel="[\"nofollow\"]"}
- [https://supernove.hatenadiary.jp/](https://supernove.hatenadiary.jp/){rel="[\"nofollow\"]"}
- [https://note.com/kmiura55](https://note.com/kmiura55){rel="[\"nofollow\"]"}

#undefined
:::lae-activities
    ::::lae-activity{date="2023/11/2"}
    [AWS Community Builder](https://builder.aws.com/connect/community/community-builders){rel="[\"nofollow\"]"}  

    Type: **表彰**
    ::::

    ::::lae-activity{date="2020/1/1"}
    IBM Champion 2020  

    Type: **表彰**
    ::::

    ::::lae-activity{date="2019/9/1"}
    Call for Code リージョンファイナリスト  

    Type: **表彰**
    ::::

    ::::lae-activity{date="2019/7/1"}
    LINE Things×kintoneハンズオン  

    Type: **イベント開催(LINE API)**
    ::::

    ::::lae-activity{date="2019/3/1"}
    中京テレビハッカソン「Hack-Chu」LINE賞、IBM賞、Seeed賞  

    Type: **表彰**
    ::::

    ::::lae-activity{date="2018/10/1"}
    One Japan Tokai Hackathon LINE賞  

    Type: **表彰**
    ::::
  :::
::
