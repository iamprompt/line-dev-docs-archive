---
title: LINE Beacon no longer supported on several LINE versions
navigation: true
description: >-
  As announced on March 10, 2020, LINE Beacon is no longer supported on the
  following LINE versions:
meta: '{"date":"2020-03-26 00:00 UTC","tags":"Messaging API","locale":"en"}'
path: /en/_partials/news/2020/2020-03-26-line-beacon-eol
__hash__: O18KdoDQkEmgwos3GND5YNnnf32GbEv5PIyQ85Y9inI
seo:
  title: LINE Beacon no longer supported on several LINE versions
  description: >-
    As announced on March 10, 2020, LINE Beacon is no longer supported on the
    following LINE versions:
---

As announced on [March 10, 2020](/news/2020/03/10/line-beacon-deprecation/), LINE Beacon is no longer supported on the following LINE versions:

- Japan: Any version before LINE 8.17.0
- Taiwan: Any version before LINE 9.14.0
- Thailand: Any version before LINE 9.8.0
- Indonesia: Any version before LINE 9.8.0 (no change)

### Influence

Users that use these versions of LINE will not be detected when they enter or exit the reception area of a beacon.

- Webhook beacon events are not sent to bot apps.
- The beacon banner is not displayed for the corresponding user.

For more information about LINE Beacon, see [Using Beacons](/docs/messaging-api/using-beacons/).

We apologize for any inconvenience this may have caused. LINE will continue to improve the quality of its services to its customers. Thank you for your understanding.
