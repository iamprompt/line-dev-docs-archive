---
title: Line Url Scheme
navigation: true
description: >-
  You can open Sticker Shop, LIFF app or camera with the LINE URL scheme. The
  LINE URL scheme works with LINE Official Accounts too. You can let users see
...
You can open Sticker Shop, LIFF app or camera with the LINE URL scheme. The LINE URL scheme works with LINE Official Accounts too. You can let users see LINE contents from [rich menus](/docs/messaging-api/using-rich-menus/) with the [action](/reference/messaging-api/#uri-action) to open the LINE URL scheme.
...
| URL schemes beginning with `https://liff.line.me/`    | URL scheme for opening the [LIFF app](/docs/liff/overview/)                            |
| URL schemes beginning with `https://miniapp.line.me/` | URL scheme for opening the [LINE MINI App](/docs/line-mini-app/discover/introduction/) |

::admonition{title=""line://" is deprecated" type="warning"}
...
---
title: >-
  LY Corporation doesn't provide a URL scheme for launching native apps other
  than LINE
type: note
---
LY Corporation offers no URL scheme to launch native apps other than LINE. However, if a native app from another company has a URL scheme for launching the native app, you can use the URL scheme in the URI action object for [rich menus](/docs/messaging-api/using-rich-menus/) or [Flex Messages](/docs/messaging-api/using-flex-messages/).
::
...
::admonition
---
title: Restriction on opening the camera or the camera roll
type: note
---
...
::admonition
---
title: Restriction on opening the location information screen
type: note
---
...
You can specify either [a Basic ID or a Premium ID](https://help.linebiz.com/lineadshelp/s/article/L000001191?language=ja){rel="[\"nofollow\"]"} (only available in Japanese) as the LINE ID for your LINE Official Account.
::

::admonition
---
title: Verify the LINE ID of the LINE Official Account
type: tip
---
...
::admonition{title="LINE URL schemes on a PC browser" type="tip"}
When users open `https://line.me/R/ti/p/{Percent-encoded LINE ID}` from a PC, they see the public URL of the business profile of the LINE Official Account (e.g. [LINE FRIENDS profile page](https://line.me/R/ti/p/@linecharacter){rel="[\"nofollow\"]"}) or only a QR code. What users see depends on these conditions:
...
![](/media/messaging-api/using-line-url-scheme/settings.png){className="[\"border\",\"w-fix-240\"]"}
...
| LINE URL scheme                                        | Description                                                                                                                                                                                                    |
| ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| https://[line.me/R/shop/sticker/detail/`{package_id}`] | Opens a Sticker set info screen. Specify `{package_id}` to the number specified in the sticker's page URL in [LINE STORE](https://store.line.me/){rel="[\"nofollow\"]"}.                                       |
| https://[line.me/R/shop/category/`{category_id}`]      | Opens a popularity ranking for the given category. Specify `{category_id}` to the number specified in the category page URL in [LINE STORE](https://store.line.me/){rel="[\"nofollow\"]"} > Official stickers. |
| https://[line.me/R/shop/sticker/author/`{author_id}`]  | Opens a list of sticker sets from the given author. Specify `{author_id}` to the number specified in the creator's page URL in [LINE STORE](https://store.line.me/){rel="[\"nofollow\"]"}.                     |
...
::admonition{title="Create your own sticker set" type="tip"}
To create your own sticker sets for users, visit [LINE Creators Market](https://creator.line.me/en/){rel="[\"nofollow\"]"} and use the [LINE Sticker Maker](https://creator.line.me/en/stickermaker/){rel="[\"nofollow\"]"} app.
::
...
| LINE URL scheme                                         | Description                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| https://[line.me/R/shop/theme/detail?id=`{product_id}`] | Opens a Theme info screen. Specify `{product_id}` to the ID specified in the theme page URL in [LINE STORE](https://store.line.me/){rel="[\"nofollow\"]"}. For example, if you open [Matte White](https://store.line.me/themeshop/product/0bac8fed-4c75-40c5-9982-e9ecc3b9d191/en){rel="[\"nofollow\"]"}`https://store.line.me/themeshop/product/0bac8fed-4c75-40c5-9982-e9ecc3b9d191/en`, specify `0bac8fed-4c75-40c5-9982-e9ecc3b9d191`. |

### Opening a LIFF app

With the LINE URL scheme, you can let users open a LIFF app. A LIFF app is a web app built using the [LINE Front-end Framework (LIFF)](/docs/liff/overview/).

![](/media/messaging-api/using-line-url-scheme/liff-app.png){className="[\"border\",\"w-fix-240\"]"}
...
For more information on the process of opening a LIFF app, see [Opening a LIFF app](/docs/liff/opening-liff-app/) in the LIFF documentation.
...
With the query parameters, you can let users open a URL in an :glossary-tooltip[[external browser](/glossary/#external-browser)]{glossary-id="external-browser"} instead of :glossary-tooltip[[LINE's in-app browser](/glossary/#line-iab)]{glossary-id="line-iab"}.

::admonition
---
title: These query parameters aren't supported on LIFF apps
type: note
---
