...
:markdown-controlsThis LINE Beacon specification is for corporate users who wish to deploy beacon devices to use LINE Beacon. Your beacon devices must comply with this specification.

Unlike [LINE Simple Beacon](https://github.com/line/line-simple-beacon){rel="[\"nofollow\"]"} packets, LINE Beacon packets have the [secure message](#generating-secure-message) field as a mechanism for security.

## Requirements for LINE Beacon compliant devices

A beacon device compliant with the LINE Beacon specification supports Bluetooth® Low Energy Version 4.0 and Apple's [iBeacon](https://developer.apple.com/ibeacon/){rel="[\"nofollow\"]"} and can advertise LINE Beacon packets. Specifically, the device must meet the following requirements:
...
| 12-16 | HWID                        | Device-specific value | A 5-byte unique ID of the beacon device. For more information, see [HWID](#hwid).                                                                                                                                                                                               |
...
| 18-21 | Message authentication code | Variable              | A 4-byte code for [message authentication](#step-one-generate-message-auth-code)                                                                                                                                                                                                |
| 22-23 | Masked timestamp            | Variable              | A 2-byte [masked timestamp](#step-two-generate-masked-timestamp)                                                                                                                                                                                                                |
| 24    | Battery level               | Variable              | Remaining [battery level](#battery-level)                                                                                                                                                                                                                                       |
...
The result of concatenation is a secure message. See [Sample code and data for generating secure messages](/docs/messaging-api/secure-message-sample/) to develop and test for your beacon device to generate secure messages.
