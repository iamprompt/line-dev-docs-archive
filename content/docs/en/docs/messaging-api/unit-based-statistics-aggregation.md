...
__hash__: qZw5Swh1U18NjE_-HpJp46BhexSNKaw5g9J8pwGuCGg
seo:
  title: Get statistics of sent messages
  description: null
---
...
:markdown-controlsWith the Messaging API, you can get statistics on how users interact with messages sent from a LINE Official Account. How you get statistics depends on the type of message you send.

- [Statistics for messages sent to all friends or to friends selected using audiences or attributes](#get-statistics-on-narrowcast-or-broadcast)
  - Broadcast messages
  - Narrowcast messages
- [Statistics for messages sent by specifying particular friends, group chats, or phone numbers](#get-statistics-on-push-multicast-or-line-notification-messages)
  - Push messages
  - Multicast messages
...
You can get statistics for each send request on how users interact with [narrowcast messages](/reference/messaging-api/#send-narrowcast-message) or [broadcast messages](/reference/messaging-api/#send-broadcast-message). For more information, see [Get user interaction statistics](/reference/messaging-api/#get-message-event) in the Messaging API reference.

## Statistics for messages sent by specifying particular friends, group chats, or phone numbers

You can get statistics per unit on how users interact with [push messages](/reference/messaging-api/#send-push-message), [multicast messages](/reference/messaging-api/#send-multicast-message), or [LINE notification messages](/docs/partner-docs/line-notification-messages/overview/).
...
::admonition
---
title: Specify a unit name for LINE notification messages
type: tip
---
In addition to push messages and multicast messages sent with the Messaging API, you can specify a unit name when sending LINE notification messages, an option for corporate users. For more information, see [Get statistics of LINE notification messages](/docs/partner-docs/line-notification-messages/statistics/) in the LINE notification messages documentation.
::

As illustrated below, you can get statistics for each unit by assigning a unit name and sending a message.

![](/media/news/customAggregationUnits_en.png){className="[\"w-fix-920\"]"}
...
::admonition{title="Assigning or changing unit name later" type="tip"}
...
::admonition{title="Regarding the unit name limit" type="tip"}
...
You can get user interaction statistics for messages sent with unit names by using the [Get statistics per unit](/reference/messaging-api/#get-statistics-per-unit) endpoint. Here is an example request to get statistics of a unit named `promotion_a`:
...
In addition, you can get a list of unit names assigned during this month by using the [Get a list of unit names assigned during this month](/reference/messaging-api/#get-a-list-of-unit-names-assigned-during-this-month) endpoint. There is no endpoint to check unit names assigned before the current month.
...
![](/media/messaging-api/insight/new-item-message-example-en.png){className="[\"w-fix-360\",\"border\"]"}
