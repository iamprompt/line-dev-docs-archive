...
:markdown-controlsEvery time a user adds your LINE Official Account as a friend or sends it a message, the LINE Platform sends an HTTP POST request with a webhook event object to the webhook URL (bot server) you register in the [LINE Developers Console](/console/).
...
::admonition{title="Security warning" type="warning"}
...
::admonition
---
title: We recommend processing events asynchronously
type: tip
---
...
Users can't send a [template message](/reference/messaging-api/#template-messages) or a [Flex Message](/reference/messaging-api/#flex-message) from the LINE app. However, developers can use [`liff.sendMessages()`](/reference/liff/#send-messages) to send messages on behalf of the users to the currently open chat screens where the LINE MINI Apps or LIFF apps are open.

When a template message or a Flex Message is sent from the user using `liff.sendMessages()`, no webhook is sent from the LINE Platform. For all other [message types](/docs/messaging-api/message-types/), a webhook is sent.
...
| ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Beacon event](/reference/messaging-api/#beacon-event)             | When a user enters the reception range of a Beacon. You can reply to this event. For more information, see [Use beacons with LINE](/docs/messaging-api/using-beacons/).                                        |
| [Account link event](/reference/messaging-api/#account-link-event) | When a user links their LINE account with their account of your service (as a provider). You can reply to this event. For more information, see [User account linking](/docs/messaging-api/linking-accounts/). |

## Processing on receipt of unsend event

Users can unsend a message for a limited time after sending it.

When a user unsends a sent message, an [unsend event](/reference/messaging-api/#unsend-event) is sent to the bot server. When the unsend event is received, we recommend that service providers respect the user's intent to unsend a sent message and handle the message appropriately with the utmost care so that the target message can't be seen or used in the future.
...
::admonition{title="Check before enabling webhook redelivery" type="note"}
...
The content of a redelivered [webhook event object](/reference/messaging-api/#webhook-event-objects) is the same as the original webhook event object, except for the value of `deliveryContext.isRedelivery`. Values such as webhook event ID and reply token remain unchanged.

The reply token included in a redelivered webhook event object can be used except in certain cases. For more information on reply tokens, see [Reply token](/reference/messaging-api/#send-reply-message-reply-token) in the Messaging API reference.

### Enable webhook redelivery

By default, webhook redelivery is disabled. To enable webhook redelivery:

1. Open the channel settings screen from the [LINE Developers Console](/console/).
...
![](/media/messaging-api/receiving-messages/enable-webhook-redelivery-en.png){className="[\"border\",\"w-fix-480\"]"}
...
::admonition{title="May not be able to redeliver webhooks" type="note"}
...
For more information, see [Check webhook error causes and statistics](/docs/messaging-api/check-webhook-error-statistics/).

## Get user-sent content with webhook

You can get content sent by users with a message ID in the [webhook](/reference/messaging-api/#webhooks). The types of content that can be retrieved are as follows:
...
- You can get the text that users sent through the [text](/reference/messaging-api/#wh-text) message object of the webhook. There is no API available to get the text again after receiving the webhook.
::

### Get images, videos, audio, and files

You can get [images](/reference/messaging-api/#wh-image), [videos](/reference/messaging-api/#wh-video), [audio](/reference/messaging-api/#wh-audio), and [files](/reference/messaging-api/#wh-file) that users send, with a message ID in webhook.
...
For more information, see [Get content](/reference/messaging-api/#get-content) in the Messaging API reference.
...
You can get a user's LINE profile (user's display name, user ID, profile image URL, status message, etc.) with the user's ID contained in a [webhook](/reference/messaging-api/#webhooks).
...
For more information, see [Get profile](/reference/messaging-api/#get-profile) in the Messaging API reference.
