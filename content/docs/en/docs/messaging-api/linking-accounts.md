...
__hash__: 7pz5xrEbYONstBD7Hxa3cWxgait566NK1tw-ssEAVhM
seo:
  title: User account linking
  description: null
---
...
![](/media/messaging-api/linking-accounts/sequence.png){className="[\"border\",\"w-fix-600\"]"}
...
You need a link token to link a user's LINE and your service accounts. To [issue a link token](/reference/messaging-api/#issue-link-token), send an HTTP POST request to the `/bot/user/{userId}/linkToken` endpoint.
...
Your bot server sends a URL for the user to link accounts. For example, the linking URL can be specified in the [URI action](/docs/messaging-api/actions/#uri-action) object of a [template message](/docs/messaging-api/message-types/#template-messages). In the linking URL, add a query parameter with the linking token from step 1.
...
- User verification succeeds: The LINE Platform sends an [account link event](/reference/messaging-api/#account-link-event) to your bot server. The `result` property value of the event is `ok`.
- User verification fails: The LINE Platform sends an [account link event](/reference/messaging-api/#account-link-event) to your bot server. The `result` property value of the event is `failed`.
...
For example, the Messaging API lets you customize [rich menus](/docs/messaging-api/rich-menus-overview/) by user. You can display a menu to link accounts to those who are yet to link accounts. Likewise, you can display a menu to unlink accounts to those who linked their accounts.
