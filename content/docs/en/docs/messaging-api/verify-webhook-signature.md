...
::admonition
---
title: We recommend that you verify the signature of the webhook
type: tip
---
Verifying the webhook signature is an important security measure. Verifying the webhook signature is also recommended in the [Messaging API development guidelines](/docs/messaging-api/development-guidelines/#verify-webhook-signature).
::

::admonition
---
title: The LINE Platform doesn't disclose IP addresses
type: tip
---
...
Open the channel's **Basic settings** tab in the [LINE Developers Console](/console/) and get the channel secret. You need Admin privileges for the channel to get the channel secret.

![](/media/messaging-api/verify-webhook-signature/channel-secret-en.png){className="[\"border\"]"}
...
To reissue the channel secret, click **Issue** in the **Basic Settings** tab in the [LINE Developers Console](/console/). If you are concerned that your channel secret has been compromised, reissue the channel secret. You need Admin privileges for the channel to reissue a channel secret.
...
1. [The LINE Platform sends a webhook to the bot server](#line-platform-sends-webhook-request)
2. [The bot server receives the webhook](#receiving-webhook-request)
3. [The webhook signature is verified on the bot server](#signature-validation)
...
1. Generates a signature with HMAC-SHA256 using the [webhook event](/reference/messaging-api/#webhook-event-objects) as input data and the channel secret as the hash key.
...
Don't modify the signature (`x-line-signature`) in the [request header](/reference/messaging-api/#request-headers) of the received webhook, or the request body string, and store them as-is in the memory or database.

::admonition
---
title: Don't modify the data before verifying the signature
type: note
---
...
If the signatures don't match even though the webhook was sent from the LINE Platform, there may be an error in the way the signature is verified on the bot server. For more information, see [Common signature verification failures and their solutions](#common-signature-verification-failures-and-their-solutions).

## Signature verification procedure

Use the `openssl` command to review the signature verification procedure.

First, open the **Messaging API** tab for the channel from the [LINE Developers Console](/console/) and click **Verify** for the webhook URL to send a confirmation webhook from the LINE Platform.

![Click Verify to send a webhook to confirm communication.](/media/news/webhook-url-verify-button.png){className="[\"border\",\"w-fix-320\"]"}

1. Webhook request body sent to the bot server
```json
{"destination":"U8e742f61d673b39c7fff3cecb7536ef0","events":[]}
```
2. Signature (`x-line-signature`) of the webhook sent to the bot server
```text
GhRKmvmHys4Pi8DxkF4+EayaH0OqtJtaZxgTD9fMDLs=
```
3. The channel secret for that channel
```text
8c570fa6dd201bb328f1c1eac23a96d8
```
4. Command to verify signature on bot server
```sh
...
```
5. Signature generated by the bot server
```text
GhRKmvmHys4Pi8DxkF4+EayaH0OqtJtaZxgTD9fMDLs=
```
...
In actual development, you can easily verify the signature by using the [LINE Messaging API SDK](/docs/messaging-api/line-bot-sdk/). See [Signature validation](/reference/messaging-api/#signature-validation) in the Messaging API Reference for implementation examples in each language.
...
- [Webhook changed before reaching the bot server](#modified-webhook-request-before-it-reaches-the-bot-server)
- [Parsing and deserializing webhooks](#parsing-or-deserializing-webhook-request)
- [Incorrect formatting of the webhook request body string (JSON)](#formatted-webhook-event)
- [An algorithm other than HMAC-SHA256 was used to verify the signature](#used-incorrect-algorithm-for-signature-validation)
- [Used a channel secret for a different channel](#used-wrong-channel-secret)
- [Another developer has reissued the channel secret](#reissued-channel-secret)
- [Interpreted escape characters](#interpreted-escape-characters)
- [Character encoding used when processing webhooks wasn't UTF-8](#non-utf8-encoding-for-webhook-processing)
...
{"destination":"U8e742f61d673b39c7fff3cecb7536ef0","events":[]}
```
2. Deserialize and output the webhook request body
```python
decoded_data = json.loads('{"destination":"U8e742f61d673b39c7fff3cecb7536ef0","events":[]}')
print(decoded_data)
```
3. Deserialization has changed double quotes to single quotes and/or introduced spaces
```python
{'destination': 'U8e742f61d673b39c7fff3cecb7536ef0', 'events': []}
```
...
{"destination":"U8e742f61d673b39c7fff3cecb7536ef0","events":[]}
```
2. Signature verification fails when webhook request body string (JSON) is formatted
```json
{
...
Check the value of the Channel secret in the **Basic settings** tab of the [LINE Developers Console](/console/).

### Another developer has reissued the channel secret

When a new channel secret is issued in the **Basic settings** tab of the [LINE Developers Console](/console/), the previous channel secret becomes invalid.
...
Check the current channel secret value in the **Basic settings** tab in the [LINE Developers Console](/console/). If the channel secret was reissued, the channel secret used for signature verification on the bot server must be replaced with the new one.
