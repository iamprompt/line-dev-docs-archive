...
:markdown-controlsUsing the video component of Flex Message, you can display a video in the hero [block](/docs/messaging-api/flex-message-elements/#block). For more information on sending a Flex Message, see [Send Flex Messages](/docs/messaging-api/using-flex-messages/).

:toc## Requirements to include videos in Flex Messages

To include a [video](/docs/messaging-api/flex-message-elements/#video) component in a Flex Message, you must:

- Specify the type of a hero [block](/docs/messaging-api/flex-message-elements/#block) as `video`.
...
Using the `action` property, you can specify a [URI action](/reference/messaging-api/#uri-action). This action lets users open a URL in LINE's in-app browser or call a given number with the device's call app. The label for a URI action will be displayed in the following three places:
...
![Video component example](/media/messaging-api/flex-message-elements/video-sample.png){className="[\"w-fix-280\"]"}
...
Flex Messages that contain only a video looks like [video messages](/reference/messaging-api/#video-message). With Flex Message, you can build a message with a video in more complex layout.
...
![Screen when a video finishes playing](/media/messaging-api/create-flex-message-including-video/auto-play-finished-en.png){className="[\"w-fix-280\"]"}
...
If the version of LINE is lower than the version that supports the video component, the component specified as the value of the `altContent` property will be displayed. Use a [box](/reference/messaging-api/#box) component or an [image](/reference/messaging-api/#f-image) component for the `altContent` property.

## Videos in Flex Message Simulator

You can't preview a video on [Flex Message Simulator](/flex-simulator/). Instead, the alternative content you set is displayed in the preview area of Flex Message Simulator, like in [Display on version of LINE that doesn't support the video component](#alt-content).
...
![The Send... button on Flex Message Simulator](/media/messaging-api/create-flex-message-including-video/send.png){className="[\"w-fix-360\",\"bg-border\"]"}
