...
:markdown-controlsIn this tutorial, you'll learn how to send a message with the Messaging API and Node.js through a reply bot.
...
This tutorial assumes that you have a basic knowledge of JavaScript and Node.js. Before you continue with this tutorial, we recommend that you read through [Messaging API overview](/docs/messaging-api/overview/).

::admonition{title="This tutorial doesn't use an SDK" type="tip"}
To help you learn the Messaging API, this tutorial shows you how to use the Messaging API with Node.js, but without the SDK provided by LY Corporation. To speed things up and write less lines of code for your project with Node.js, try the [LINE Messaging API SDK for nodejs](https://line.github.io/line-bot-sdk-nodejs/){rel="[\"nofollow\"]"}.
::
...
- A [LINE Developers Console](/console/) account: Log in to the LINE Developers Console with your LINE account or business account and [create a developer account](/docs/line-developers-console/login-account/#register-as-developer) if you don't already have one.
- A [Heroku](https://www.heroku.com/){rel="[\"nofollow\"]"} account::admonition{title="Heroku's free plan has been discontinued" type="note"}
Heroku's free plan is discontinued as of November 27, 2022. To try this tutorial for free, use a different platform. For more information, see [Heroku’s Next Chapter](https://www.heroku.com/blog/next-chapter/){rel="[\"nofollow\"]"}.
::
...
![Welcome page](/media/messaging-api/node-js-sample/welcome-page.png){className="[\"border\",\"w-fix-480\"]"}
...
Install the package [Express.js](https://expressjs.com/){rel="[\"nofollow\"]"} with the command below. This is a lightweight Node.js web server framework we'll use in our project.
...
Now, add environment variables to simplify configuration process and secure credentials. The `process.env.PORT` variable specifies which port the server is to listen to. The `process.env.LINE_ACCESS_TOKEN` has the :glossary-tooltip[[channel access token](/glossary/#channel-access-token)]{glossary-id="channel-access-token"} required to call the Messaging API. In `index.js`, add these settings under imported packages:
...
It's time to implement the core feature of our reply bot, sending a reply to a user's message. The first thing we need to do is to detect when users send a message to you. We detect when we receive at the webhook URL a [message event](/reference/messaging-api/#message-event) with the `type` property set to `message`.

::admonition
---
title: Verify the signature to publish the bot for production use
type: warning
---
...
For more information on how to verify signatures, see [Verify signature](/docs/messaging-api/receiving-messages/#verify-signature).
::

To send a reply to the user, we use the [send reply message](/reference/messaging-api/#send-reply-message) endpoint. From the `app.post()` in the `index.js` file, call the send reply message endpoint (`https://api.line.me/v2/bot/message/reply`). Replace `app.post` with this code. Check the comments in the code below for detailed explanation:
...
To use the Messaging API, you need to have a Messaging API channel and register your webhook URL. [Create a channel](/docs/messaging-api/getting-started/) if you don't have one yet.

From your Messaging API channel page on the LINE Developers Console, open the **Messaging API** tab and issue a [channel access token](/docs/basics/channel-access-token/). We'll use this token when we [deploy our bot to Heroku](#deploy-on-heroku).

![Channel access token section in a Messaging API channel](/media/messaging-api/node-js-sample/channel-access-token-en.png){className="[\"border\",\"w-fix-720\"]"}
...
::admonition{title="Did you forget the Heroku URL?" type="tip"}
If you forgot or lost your Heroku URL, you can get the URL from the [Heroku Dashboard](https://dashboard.heroku.com/){rel="[\"nofollow\"]"}.
::

Lastly, enable the **Use webhook** settings.

!["Enable webhook" setting in Messaging API tab](/media/messaging-api/node-js-sample/enable-webhook-en.png){className="[\"border\",\"w-fix-480\"]"}
...
- Add [rich menus](/reference/messaging-api/#rich-menu) to show tappable options to users.
- Respond to a user based on the [action object](/reference/messaging-api/#action-objects) you receive when a user triggers an action.
- [Get a user's profile](/reference/messaging-api/#get-profile) and send a customized message based on the profile information.

As introduced in the beginning of this tutorial, the [LINE Messaging API SDK for nodejs](https://line.github.io/line-bot-sdk-nodejs/){rel="[\"nofollow\"]"} helps you to build a bot much quicker. Have a go at it!

## Learn more

- [Messaging API reference](/reference/messaging-api/)
- [Messaging API overview](/docs/messaging-api/overview/)
- [https.request specification (Node.js)](https://nodejs.org/api/https.html#https_https_request_options_callback){rel="[\"nofollow\"]"}
