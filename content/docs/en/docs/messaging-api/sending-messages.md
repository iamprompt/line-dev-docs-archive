...
:markdown-controlsWith the Messaging API, you can make your bot send messages to users. At any time, you can initiate to send a message to a user and reply to a user's message. You can also use various message types.

| dummy           | dummy                                                                                                                                                                                                                                                                            |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Messaging types | - Reply message - Push message: One-to-one - Multicast message: One-to-many (Targets a list of user IDs) - Narrowcast message: One-to-many (Targets a segmented list) - Broadcast message: One-to-many (Targets all friends)                                                     |
| Message types   | - Text message - Text message (v2) - Sticker message - Image message - Video message - Audio message - Location message - Imagemap message - Template message - Flex message  For more information about message types, see [Message types](/docs/messaging-api/message-types/). |
...
When a user adds your LINE Official Account as a friend or sends a message to your LINE Official Account, you can reply with the Messaging API. Set the `replyToken` property to the reply token you receive through a [webhook event](/reference/messaging-api/#webhook-event-objects) from the user's action. In a single request, you can send up to five [message objects](/reference/messaging-api/#message-objects).
...
type: tip
---
After your LINE Official Account receives a message from a user, the response may takes some time due to message preparation or reservation processing. In such cases, you can visually tell the user that you want them to wait by displaying a loading animation. For more information, see [Display a loading animation](/docs/messaging-api/use-loading-indicator/).
::
...
For more information, see [Send reply message](/reference/messaging-api/#send-reply-message) in the Messaging API reference.
...
| --------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Push message](/reference/messaging-api/#send-push-message)           | Sends a message to a user, group chat, and multi-person chat. For example, you can use this to notify a user that an item they bought on your shopping site is shipped.                                                                                    |
| [Multicast message](/reference/messaging-api/#send-multicast-message) | Sends a message to multiple users at once. The difference between this and narrowcast is that you specify target recipients with a user ID. For example, you can use this to notify all your shopping site members of a new feature.                       |
...
| [Broadcast message](/reference/messaging-api/#send-broadcast-message) | You can send the same message to all users who are friends with your LINE Official Account.                                                                                                                                                                |

You can send up to five [message objects](/reference/messaging-api/#message-objects) in a single request.

::admonition{title="How to count the number of messages" type="tip"}
The number of messages counted as sent is the number of people you send the message to. The number of [message objects](/reference/messaging-api/#message-objects) you specify in a single request has no effect on the number of messages sent. Suppose you sent a push message with four message objects in a single request to a chat room containing five people. Here, the number of sent messages is five.
...
| Users identifiable by a [user ID](#user-id) or Identifier for Advertisers (IFA)                                | - [Audience for uploading user IDs (by JSON)](/reference/messaging-api/#create-upload-audience-group) - [Audience for uploading user IDs (by file)](/reference/messaging-api/#create-upload-audience-group-by-file)                                         |
| Users who clicked a URL in a message you sent                                                                  | [Message click audience](/reference/messaging-api/#create-click-audience-group)                                                                                                                                                                             |
| Users who opened a message you sent                                                                            | [Message impression audience](/reference/messaging-api/#create-imp-audience-group)                                                                                                                                                                          |
| Users who received a narrowcast message                                                                        | Specify the request ID of the narrowcast message previously delivered in the redelivery object of the [recipient object](/reference/messaging-api/#narrowcast-recipient).                                                                                   |
| Users with specific tags in a chat                                                                             | Chat tag audience. Use the [LINE Official Account Manager](https://manager.line.biz/){rel="[\"nofollow\"]"} to create one.                                                                                                                                  |
| Users who added your LINE Official Account as a friend through a specific path                                 | Friend path audience. Use the [LINE Official Account Manager](https://manager.line.biz/){rel="[\"nofollow\"]"}.                                                                                                                                             |
| Users who reserved a visit before                                                                              | Reservation audience. Use the [LINE Official Account Manager](https://manager.line.biz/){rel="[\"nofollow\"]"} to create one.                                                                                                                               |
| Users who viewed a rich menu                                                                                   | Rich menu impression audience. Use the [LINE Official Account Manager](https://manager.line.biz/){rel="[\"nofollow\"]"} to create one.                                                                                                                      |
| Users who clicked on a rich menu                                                                               | Rich menu click audience. Use the [LINE Official Account Manager](https://manager.line.biz/){rel="[\"nofollow\"]"} to create one.                                                                                                                           |
| Users narrowed down with LINE Tag tracking information                                                         | Web traffic audience (LINE Tag). Use the [LINE Official Account Manager](https://manager.line.biz/){rel="[\"nofollow\"]"} or the [LINE Ad Manager](https://admanager.line.biz/){rel="[\"nofollow\"]"} to create one.                                        |
| Users narrowed down with Tracking Tag information                                                              | Web traffic audience (Tracking Tag). Use the [LINE Official Account Manager](https://manager.line.biz/){rel="[\"nofollow\"]"} to create one.                                                                                                                |
| Users who viewed a video you sent                                                                              | Video view audience. Use the [LINE Ad Manager](https://admanager.line.biz/){rel="[\"nofollow\"]"} to create one.                                                                                                                                            |
| Users who participated in given events within the app (e.g. opening the app, making a purchase within the app) | App event audience. Use the [LINE Ad Manager](https://admanager.line.biz/){rel="[\"nofollow\"]"} to create one.                                                                                                                                             |
| Users who clicked an image you sent                                                                            | Image click audience. Use the [LINE Ad Manager](https://admanager.line.biz/){rel="[\"nofollow\"]"} to create one.                                                                                                                                           |
| Users who saw a [beacon banner](/docs/messaging-api/using-beacons/#beacon-banner)                              | LINE Beacon Network ad impression audience. Use the [LINE Ad Manager](https://admanager.line.biz/){rel="[\"nofollow\"]"} to create one. LINE Beacon Network ad impression audience is only available for LINE Official Accounts created by users in Taiwan. |
...
:interactive-svg{src="/media/news/redeliver-narrowcast-en.svg" border}::admonition{title=""There weren't enough recipients" error" type="note"}
...
To prevent someone from guessing the recipients' attributes, you can't send narrowcast messages if the number of recipients is less than the required minimum. For more information, see [Restrictions on sending messages using attributes and audiences](/reference/messaging-api/#send-narrowcast-message-restrictions).
::

For more information on the audience object and redelivery object, see [Recipient object](/reference/messaging-api/#narrowcast-recipient) in the Messaging API reference.
...
For more information, see [Demographic filter object](/reference/messaging-api/#narrowcast-demographic-filter) in the Messaging API reference.
...
---
title: >-
  You can use operator objects to specify target recipients in a nested
  structure
type: tip
---
...
For more information, see [Limit objects](/reference/messaging-api/#send-narrowcast-limit) in the Messaging API reference.
...
For more information, see [Sending narrowcast messages](/reference/messaging-api/#send-narrowcast-message) in the Messaging API Reference.

### Check the status of a narrowcast message

Narrowcast messages are sent asynchronously in the background. To find out whether the narrowcast message was sent successfully, execute the [Get narrowcast message status](/reference/messaging-api/#get-narrowcast-progress-status) endpoint as shown in the example below.
...
![If the message to be quoted does not exist, it will be displayed as "Message unavailable."](/media/messaging-api/sending-messages/delete-quoted-message-en.png){className="[\"border\",\"w-fix-480\"]"}
