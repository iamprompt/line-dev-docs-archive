...
:markdown-controlsFlex Messages are messages that can be freely customized based on [CSS Flexible Box (CSS Flexbox)](https://www.w3.org/TR/css-flexbox-1/){rel="[\"nofollow\"]"}. Depending on your needs, you can adjust the size of the message, allocate text, images, and icons in specific locations, and add interactive buttons.

In this tutorial, you'll learn how to use [Flex Message Simulator](/flex-simulator/) to create a digital business card. Flex Message Simulator is a tool to help you brainstorm, design, and prototype Flex Messages **without writing code**. If you are new to Flex Messages, see [Sending Flex Messages](/docs/messaging-api/using-flex-messages/) first.

## Goal

The outcome of this tutorial is a digital business card as shown below. You can see the outcome defined in JSON from this :file-link[download link]{file="/media/code-samples/flex-message-simulator-example.json"}. But we recommend that you to go through this tutorial to become familiar with Flex Message Simulator. We guarantee, this tool will become quite handy to deal with a limitless number of use cases for Flex Messages.

![Final Output](/media/messaging-api/using-flex-message-simulator/en-final-output.png){className="[\"border\",\"w-fix-280\"]"}

## Before you start

To follow this tutorial, we recommend that you read [Messaging API overview](/docs/messaging-api/overview/) and [Send Flex Messages](/docs/messaging-api/using-flex-messages/), before you get started. Also, read on this section to learn about Flex Message Simulator if you are new to the simulator. If you're familiar with the simulator, [start with the tutorial](#select-flex-message) right now.
...
First, open [Flex Message Simulator](/flex-simulator/). If you aren't logged in to the [LINE Developers Console](/console/), you're prompted to log in. If you have a LINE Developers account, log in with your account. If not, click **Create an account** and create one.
...
::video{width="883" height="381" controls="true"}
...
::admonition{type="note" title="About layout"}
...
![View as JSON](/media/messaging-api/using-flex-message-simulator/view-as-json.png){className="[\"w-fix-360\"]"}

## Tutorial shortcut

To skip reading through the instructions and jump right to preview the outcome, :file-link[download]{file="/media/code-samples/flex-message-simulator-example.json"} the Flex Message object in JSON. To preview the outcome in Flex Message Simulator:
...
Now that we learnt about Flex Message Simulator, let's get started to create a digital business card. We only need one bubble to create a business card, so we'll set our Flex Message container to the [bubble type](/docs/messaging-api/flex-message-elements/#bubble).
...
![Select Message Type](/media/messaging-api/using-flex-message-simulator/type.png){className="[\"w-min-120\"]"}
::

For more information about the types of containers, see [container](/docs/messaging-api/flex-message-elements/#container).

## 2. Add a header

In the container we created, let's add a header to show the company name. A header is a type of a [block](/docs/messaging-api/flex-message-elements/#block) and so are hero, body, and footer. Header is mainly used to display the message subject or content heading.

![Block style example](/media/messaging-api/using-flex-message-simulator/elements.png){className="[\"w-min-480\"]"}

1. To add a header, select the **header** node in the tree view area. Click **+** at the top and click **box**.
2. Set the background color of the header. In the property area, set the **backgroundColor** field with hexadecimal color code, `#00B900` in this tutorial, and hit the Enter key. Now the header is visually distinguishable from the body block.::admonition{type="tip" title="Hit the Enter key to apply your entry"}
...
![Set Header Background Color](/media/messaging-api/using-flex-message-simulator/set-header-color.png)
3. Add a text in the header:
  1. From the tree view, click the **box [vertical]** node under **header**.::admonition{type="tip"}
  Vertical box is one of the box types for Flex Messages, that determines how the box's child components are placed within the box. For more information, see [Box component orientation](/docs/messaging-api/flex-message-layout/#box-component-orientation).
  ::
...
![Add Header Final Output](/media/messaging-api/using-flex-message-simulator/add-header-final.png)

## 3. Add an image

One way to supplement our a digital business card visually is to add an image. With Flex Message Simulator, adding or styling an image can't get easier. To add an image, we'll use the [hero block](/docs/messaging-api/flex-message-elements/#block) which is mainly used to display image type content.
...
3. To change the image, click the **image** node from the tree view. In the property area, change the value of the **url** property to the location for your image. Your image must be in portrait mode. For the tutorial, you can use :file-link[this]{file="/media/messaging-api/using-flex-message-simulator/mary.png"} image.::admonition{type="note" title="Image requirements"}
...
::admonition{type="tip" title="File size recommendation"}
...
![Add Name Final Output](/media/messaging-api/using-flex-message-simulator/en-add-name-final.png)
...
![Add Job Title Final](/media/messaging-api/using-flex-message-simulator/en-add-job-title-final.png)

## 6. Add a separator

Later on, we'll add buttons to make the business card interactive. Before we do that, let's add a [separator](/reference/messaging-api/#separator) to visually separate the information section and interactive section.

1. In the tree view, click the **box [vertical]** node under **body**.
2. Click **+** and then **separator** from the drop-down menu. A separator is created right under the job title.

![Add Separator](/media/messaging-api/using-flex-message-simulator/en-add-separator.png)

The gap between the separator and the job title is almost none. Let's give a room between the two by giving a [margin](/reference/messaging-api/#separator) to the separator. For more information about margins, see [Separator](/reference/messaging-api/#separator) in the Messaging API reference.
...
![Add Separator Final](/media/messaging-api/using-flex-message-simulator/en-add-separator-final.png)
...
We want the buttons to execute an action when pressed. Available action types for buttons are [URI action](/reference/messaging-api/#uri-action) and [postback action](/reference/messaging-api/#postback-action). For this tutorial, we'll add buttons to:
...
::admonition{type="note" title="Apply percent encoding to the URI"}
[Percent-encode](https://en.wikipedia.org/wiki/Percent-encoding){rel="[\"nofollow\"]"} the domain name, path, query parameters, and fragments in the `uri` property, with the UTF-8 encoding. For example, the final URL becomes `https://example.com/path?q=Good%20morning#Good%20afternoon` with these settings:
...
![Add URI Action Button](/media/messaging-api/using-flex-message-simulator/en-add-uri-button.png)
...
![Change Button Color](/media/messaging-api/using-flex-message-simulator/en-change-button-color.png)

### 7-2. Add a button to open a registration form made with LIFF

Let's continue to add the rest of the buttons. For the second button, we want to add a [LINE Front-end Framework (LIFF)](/docs/liff/overview/) URL to the business' registration form. You can create registration forms with LIFF and later send a new message to the user with the information you retrieved from the form. For more information on LIFF, see [Developing a LIFF App](/docs/liff/developing-liff-apps/) or [Trying the LIFF starter app](/docs/liff/trying-liff-app/).
...
![Add LIFF Button](/media/messaging-api/using-flex-message-simulator/en-add-liff-button.png)

### 7-3. Distribute the buttons

The buttons are stacked very tight to each other. It doesn't look like so, but if you change the button style to primary or secondary, you can tell right away. To distribute the buttons with some room in between, you can use [margin](/docs/messaging-api/flex-message-layout/#margin-property) or [padding](/docs/messaging-api/flex-message-layout/#padding-property) on the parent node, which is a box component for us. For this tutorial, we'll add padding:
...
When you compose a Flex Message, export the result in JSON as introduced at the [beginning of this tutorial](#copy-json). This is handy when you want to send a Flex Message with the Messaging API. For more information, see [Call the Messaging API to send the Flex Message](/docs/messaging-api/using-flex-messages/#sending-messages-with-the-messaging-api).
...
- [Flex Message layouts](/docs/messaging-api/flex-message-layout/)
