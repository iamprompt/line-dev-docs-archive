...
:markdown-controlsThis page explains how to set up a "per-user rich menu".

:toc## What is per-user rich menu
...
1. Display priority is higher than the default rich menu
  ...
  - The per-user rich menu has a higher display priority than the default rich menu. Therefore, if you've set a default rich menu for a LINE Official Account and you set a per-user rich menu for a user, the per-user rich menu will take precedence over the default rich menu. For more information, see [Display priority of rich menus](/docs/messaging-api/rich-menus-overview/#rich-menu-display).
2. Setting changes take effect immediately

  - The per-user rich menu setting takes effect immediately and changes the display without the user having to re-enter the chat screen. For more information, see [When rich menu setting changes take effect](/docs/messaging-api/rich-menus-overview/#when-setting-change-takes-effect).
...
First, create a rich menu. For more information on how to create a rich menu, see [Use rich menus](/docs/messaging-api/using-rich-menus/).

Here we use the following template image (`richmenu-template-guide-07.png`) for the rich menu. Save it on any directory.

![The template image for rich menus used in this guide](/media/messaging-api/rich-menu/richmenu-template-guide-07.png){className="[\"w-fix-320\"]"}

Run this command in your terminal, to [create a rich menu](/reference/messaging-api/#create-rich-menu):
...
Next, run this command in your terminal to [upload and attach an image to the rich menu](/reference/messaging-api/#upload-rich-menu-image).
...
For more information on getting user IDs, see [Developer gets their own user ID](/docs/messaging-api/getting-user-ids/#get-own-user-id) on [Get user IDs](/docs/messaging-api/getting-user-ids/).

### 3. Link the rich menu to the user

Once the rich menu and your user ID are ready, [link the rich menu to the user](/reference/messaging-api/#link-rich-menu-to-user). Run this command in your terminal.
...
![](/media/messaging-api/rich-menu/per-user-rich-menu-example.png){className="[\"border\",\"w-fix-320\"]"}

### 4. Unlink the rich menu from the user

Finally, [unlink the rich menu from the user](/reference/messaging-api/#unlink-rich-menu-from-user) and stop displaying the rich menu. Run this command in your terminal while displaying the chat screen opened in step 4.
