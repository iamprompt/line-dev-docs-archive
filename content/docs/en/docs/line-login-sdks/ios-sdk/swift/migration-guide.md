...
navigation: true
description: >-
  Describes noticeable changes and migration steps from an earlier version of
...
:markdown-controls## Upgrading to the latest SDK

5.0.0 is the first version of the LINE SDK for iOS Swift. This version is not compatible with the [legacy Objective-C versions](/docs/line-login-sdks/ios-sdk/deprecated/objective-c-v41/overview/). You need to change some of your code if you are upgrading to the LINE SDK for iOS Swift.

::admonition{title="Note" type="note"}
The new LINE SDK for iOS Swift is designed for Swift projects. However, you can still use the new SDK with Objective-C code. To learn how to use the SDK with Objective-C code, see [Using the SDK with Objective-C code](/docs/line-login-sdks/ios-sdk/swift/using-objc/).
::

To upgrade the SDK, it is recommended to remove all code lines related to the old SDK and perform a clean installation by following the steps in [Setting up your project](/docs/line-login-sdks/ios-sdk/swift/setting-up-project/), whether you are using a legacy version in either language. However, if you want to make changes based on your current implementation, here are some general steps:
...
3. Install the LINE SDK for iOS Swift. For detailed steps, see [Setting up your project](/docs/line-login-sdks/ios-sdk/swift/setting-up-project/).
4. Set up the channel ID and callback handling in the `AppDelegate` file.  
Call the `LoginManager.setup` method just after your app launches as below:```swift
...
Update the open URL handling as below:```swift
...
We provide a sample app which is compatible with the latest LINE SDK for iOS Swift. See our [open-source repository](https://github.com/line/line-sdk-ios-swift){rel="[\"nofollow\"]"} to find out the basic integration methods and usage.

::admonition
---
title: Logging in users into your app through LINE
type: note
---
