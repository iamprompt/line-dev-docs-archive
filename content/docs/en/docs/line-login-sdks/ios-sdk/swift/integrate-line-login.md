...
:markdown-controlsOnce you [install the SDK](/docs/line-login-sdks/ios-sdk/swift/setting-up-project/) and configure your project, you can start leveraging LINE Login to improve your app's user experience.
...
For more information on how to handle the login process using a universal link, see [Using universal links](/docs/line-login-sdks/ios-sdk/swift/universal-links-support/).
...
The LINE SDK for iOS Swift provides a predefined login button. The `LoginButton` class in the SDK is a subclass of the `UIButton` class and follows the style recommended in the [LINE Login button design guidelines](/docs/line-login/login-button/). You can add a login button to the user interface of your app to provide your users with a quick way to log in as below:
...
If an error occurs during the login process, the LINE SDK returns the `result` argument of `.failure` and an associated `LineSDKError` enumeration member. See [Handling errors](/docs/line-login-sdks/ios-sdk/swift/error-handling/) for how to get the error details from the SDK and how to handle them appropriately.

For how to design your login interface, see [LINE Login button design guidelines](/docs/line-login/login-button/) where you can also download the LINE Login button images.
...
API calls without appropriate permissions fail. For more information, see [Handling errors](/docs/line-login-sdks/ios-sdk/swift/error-handling/).
...
::admonition{title="User impersonation" type="warning"}
...
- [Get user profile](/reference/line-login/#get-user-profile)
