...
:markdown-controlsThis topic explains how to perform the following access token management tasks:
...
When making an API request through the `API` type, the LINE SDK automatically refreshes any expired access token. However, the refresh operation fails if the token has been expired for a long time. In that case, an error occurs and you need to have the user log in again. Read [Handling errors](/docs/line-login-sdks/ios-sdk/swift/error-handling/) to learn more.

::admonition{title="Access token auto-refresh" type="note"}
...
To learn more, see the [LINE Login v2.1 API reference](/reference/line-login/).
