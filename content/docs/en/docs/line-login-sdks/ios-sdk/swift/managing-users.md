...
navigation: true
description: >-
  Getting users profile data and ID tokens after login with the LINE SDK for iOS
  Swift.
...
seo:
  title: Managing users
  description: >-
    Getting users profile data and ID tokens after login with the LINE SDK for
    iOS Swift.
---

# :page-title

:markdown-controlsThis topic explains how to perform the following user management tasks:

- [Getting user profiles](#get-profile)
- [Using ID tokens to verify user identities](#get-id-token)
- [Logging out users](#logout)

::admonition{title="Creating a secure login process" type="tip"}
For general recommendations on how to securely handle user registration and login, see [Creating a secure login process between your app and server](/docs/line-login/secure-login-process/).
::
...
The [OpenID Connect](https://openid.net/developers/how-connect-works/){rel="[\"nofollow\"]"} 1.0 specification is an identity layer on top of the OAuth 2.0 protocol. With OpenID Connect, you can securely exchange information with the LINE Platform. Currently, you can get the user profile and email address from the LINE Platform by issuing ID tokens that conform to the OpenID Connect specification.

### Applying for email permission

You can request users who log in using LINE Login to grant your app the permission to get their email address. To do so, apply for the permission in the [LINE Developers Console](/console/). For more information, see [Applying for email permission](/docs/line-login/integrate-line-login/#applying-for-email-permission) in the LINE Login guide.
...
An ID token is a signed [JSON Web Token](https://datatracker.ietf.org/doc/html/rfc7519){rel="[\"nofollow\"]"}. The LINE SDK validates the token by checking its signature and validity period for you, to prevent any malformed data in it.
...
- [Verify the ID token](/reference/line-login/#verify-id-token)
