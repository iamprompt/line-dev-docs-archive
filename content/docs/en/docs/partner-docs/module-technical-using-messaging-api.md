...
- [Using the Messaging API with the channel access token of a module channel](#using-msg-api-with-module-channel-access-token)
- [Receiving a webhook](#get-webhook)
- [Getting the LINE Official Account information from a module channel](/docs/partner-docs/module-technical-using-messaging-api/#get-line-oa-info-from-module-channel)
- [Notes](#notes)

## Using the Messaging API with the channel access token of a module channel

- [User ID used in a module channel](#user-id-used-in-module-channel)
- [Channel access token of a module channel](#module-channel-access-token)
- [Calling a Messaging API endpoint](#call-msg-api-endpoint)
- [Rate limits for using the Messaging API](#rate-limits)

### User ID used in a module channel

In a module channel provided by LINE Market Place, the identifier for each user, which is the [user ID](/faq/#what-are-userid-groupid-and-roomid), is an identifier consisting of a 68-digit character string starting with the letter "L".
...
- [Short-lived channel access token](/docs/basics/channel-access-token/#short-lived-channel-access-token)
...
You can find the information you need to issue a channel access token under the **Basic settings** tab of your module channel on the [LINE Developers Console](/console/).

::admonition
---
title: Long-lived channel access tokens can't be used
type: note
---
...
The scope must be specified when attaching the module channel and permission to use it must be obtained from the LINE Official Account admin. For details, see [Request authorization from the LINE Official Account admin](/docs/partner-docs/module-technical-attach-channel/#request-auth-from-line-oa-admin).
...
::parameter-table
  ...
  You can get the user ID of the bot from the response of [attaching the module channel](/reference/partner-docs/#link-attach-by-operation-module-channel-provider) or the [Attached event](/reference/partner-docs/#attached-event).

    ::::admonition
    ---
    title: The specific header will be provided after participation
    type: note
    ---
...
The following is an example of sending a [push message](/reference/messaging-api/#send-push-message) from a module channel via Messaging API.
...
For more information about the rate limit per endpoint, see [Rate limits](/reference/messaging-api/#rate-limits) in the Messaging API reference.

### Get statistics of sent messages from a module channel

You can get statistics per aggregation unit of how users interact with [push messages](/reference/messaging-api/#send-push-message) and [multicast messages](/reference/messaging-api/#send-multicast-message) you send to multiple users.
...
For more information, see [Get statistics of sent messages](/docs/messaging-api/unit-based-statistics-aggregation/) in the Messaging API documentation.
...
For more information on scope, see [Request authorization from the LINE Official Account admin](/docs/partner-docs/module-technical-attach-channel/#request-auth-from-line-oa-admin).
::

- [`mode` property](#mode-property)
- [`destination` property](#destination-property)
- [Receiving module channel-specific webhook events](#get-module-channel-specific-webhook-events)
...
![Chat control](/media/partner-docs/module-technical/chat-control-en.png){className="[\"border\"]"}
...
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `active`                 | The channel that received the webhook event is active.   The webhook URL server that receives this webhook event can send reply messages, push messages, etc.                                                                                                                             |
| `standby`                | The channel that received the webhook event is waiting.   The webhook URL server that receives this webhook event should not send messages.      Webhook events arriving at the waiting channel won't include the `replyToken` property. Therefore, the response message isn't available. |
...
| --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Attached event](/reference/partner-docs/#attached-event)       | This event indicates that the module channel has been attached to the LINE Official Account.                                                                                                                                             |
| [Detached event](/reference/partner-docs/#detached-event)       | This event indicates that the module channel has been detached from the LINE Official Account.                                                                                                                                           |
| [Activated event](/reference/partner-docs/#activated-event)     | This event indicates that the module channel has been switched to Active Channel by calling the [Acquire Control API](/reference/partner-docs/#acquire-control-api).                                                                     |
| [Deactivated event](/reference/partner-docs/#deactivated-event) | This event indicates that the module channel has been switched to Standby Channel by calling [Acquire Control API](/reference/partner-docs/#acquire-control-api) or [Release Control API](/reference/partner-docs/#release-control-api). |
| [botSuspend event](/reference/partner-docs/#botsuspend-event)   | This event indicates that the LINE Official Account has been suspended (Suspend).                                                                                                                                                        |
| [botResumed event](/reference/partner-docs/#botresumed-event)   | This event indicates that the LINE Official Account has returned from the suspended state.                                                                                                                                               |

::admonition
---
title: How to detect a change in initiative (Chat Control)
type: tip
---
...
| Event type                                                                                                                      | Description                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [Activated event](/reference/partner-docs/#activated-event) - [Deactivated event](/reference/partner-docs/#deactivated-event) | This event is sent when a module channel that is attached to a LINE Official Account calls the Acquire Control API or Release Control API and the initiative (Chat Control) of chat will be switched.                                                                                                                                                                                                                                               |
| - [Follow event](/reference/messaging-api/#follow-event) - [Unfollow event](/reference/messaging-api/#unfollow-event)           | This event is sent when an end user blocks the LINE Official Account and adds it as a friend again.   When the end user blocks the LINE official account and adds it as a friend again, the initiative (Chat Control) will be automatically reset to the default state. If the module channel has the [Default Active](/docs/partner-docs/module-technical-chat-control/#default-active) function, it will automatically become the Active Channel. |
::

::admonition
---
title: About the suspended state (Suspend) of the LINE Official Account
type: tip
---
...
::parameter-table
  ...
  You can get the user ID of the bot from the response of the [Attach by operation of the module channel provider](#link-attach-by-operation-module-channel-provider) or the [Attached event](/reference/partner-docs/#attached-event).
    ...
    type: note
    ---
    The name (parameter name) of this header is open only to customers who participate in [LINE Marketplace](https://line-marketplace.com/jp/inquiry){rel="[\"nofollow\"]"} (only available in Japanese).
    ::::
  :::
::
...
Gets a list of basic information about the bots of multiple LINE Official Accounts that have attached module channels. For more information, see [Get a list of bots to which the module is attached](/reference/partner-docs/#get-multiple-bot-info-api) in the options for corporate customers API reference.
