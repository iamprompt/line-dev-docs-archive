...
For more information, see [Notification email example](/docs/partner-docs/error-notification/#sample-mail).
...
For more information, see the [LINE notification messages overview](/docs/partner-docs/line-notification-messages/overview/).

2025/01/28

## Source property in the webhook event of LINE notification messages has been removed

As announced on [August 9, 2024](#partner-news-20240809), as of January 28, 2025, the `source` property in the [Webhook delivery completion event](/docs/partner-docs/line-notification-messages/message-sending-complete-webhook-event/#receive-delivery-event) of LINE notification messages has been removed.
...
For more information on how to integrate Quick-fill with your LINE MINI App, see [Overview of Quick-fill](/docs/partner-docs/quick-fill/overview/).

2024/08/09

## As of January 28, 2025, source property in the webhook event of LINE notification messages will be removed

As of January 28, 2025, the `source` property in the [Webhook delivery completion event](/docs/partner-docs/line-notification-messages/message-sending-complete-webhook-event/#receive-delivery-event) of LINE notification messages will be removed.
...
Maintenance is scheduled for June 5, 2024 around 2:00 - around 3:00 (UTC+9) on the module. For more information, see the news from May 7, 2024, [Maintenance notice for Messaging API, module, and LINE Developers Console](/news/2024/05/07/maintenance-notice/).

2024/04/26

## Notification email is now be sent when webhook redelivery stopped

In [error notification](/docs/partner-docs/error-notification/) feature of the Messaging API, a notification email is now sent when webhook redelivery stopped.

If you've [enabled webhook redelivery](/docs/messaging-api/receiving-messages/#enable-webhook-redelivery) in the Messaging API channel settings, the LINE Platform will redeliver the webhook that your bot server failed to receive. However, if there's no response from your server even after a certain period of time of redelivery, the LINE Platform will stop redelivery.
...
For more information, see [When the LINE Platform stopped webhook redelivery](/docs/partner-docs/error-notification/#webhook-redelivery-stopped).

2023/11/01

## As of October 31, 2023, the audience match API has been discontinued

As announced on [July 18, 2023](#partner-news-20230718), we've discontinued the audience match API as of October 31, 2023.
...
For more information about stateless channel access tokens, see the news from August 31, 2023, [Stateless channel access token released](/news/2023/08/31/stateless-channel-access-token/).
...
- [Send a message using phone number](/reference/partner-docs/#phone-audience-match)
- [Get result of message delivery using phone number](/reference/partner-docs/#get-phone-audience-match)
...
As announced on [February 2, 2023](#partner-news-20230202), we've discontinued the Send mission stickers (v2) endpoint as of May 31, 2023. From now on, use the [Send mission stickers (v3)](/reference/partner-docs/#send-mission-stickers-v3) endpoint.

2023/04/11

## Module maintenance notice

Maintenance is scheduled for May 11, 2023 around 2:00 - around 3:00 (UTC+9) on the module. For more information, see the news from April 11, 2023, [Maintenance notice for Messaging API, module, and LINE Developers Console](/news/2023/04/11/messaging-api-module-and-console-maintenance/).
...
For more information on the Mark as read API, see [Mark as read API](/docs/partner-docs/mark-as-read/).

2023/02/02

## The Send mission stickers (v2) endpoint will be discontinued on May 31, 2023

We'll discontinue the Send mission stickers (v2) endpoint on May 31, 2023. After the discontinuation, use the [Send mission stickers (v3)](/reference/partner-docs/#send-mission-stickers-v3) endpoint.
...
The Module reference, which was previously provided as PDF files, will now be provided as documentation on the LINE Developers site. Refer to the [documentation](/docs/partner-docs/#module) and [API reference](/reference/partner-docs/#module) on Module from now on.

2022/09/28

## Information on "Feature for getting statistics per aggregation unit"

"[Feature for getting statistics per aggregation unit](/docs/messaging-api/unit-based-statistics-aggregation/)" has been integrated into Messaging API.

For more information, see the news from September 28, 2022, ["Feature for getting statistics per aggregation unit" is now available in the Messaging API](/news/2022/09/28/messaging-api-updated/).

2022/08/23

## LINE API Policy Handbook released

LINE API Policy Handbook is now available. This handbook is a document related to the [LINE Official Account Terms of Use](https://terms2.line.me/official_account_terms_jp?country=JP&lang=en){rel="[\"nofollow\"]"} and [LINE Official Account API Terms of Use](https://terms2.line.me/official_account_api_terms_jp?lang=ja&country=JP){rel="[\"nofollow\"]"} (only available in Japanese), and is intended to help you better understand and properly use the LINE API.

For more information, see [LINE API Policy Handbook](/docs/partner-docs/api-policy-handbook/).
...
By using the module, advanced features using the Messaging API can be easily added to LINE Official Account that don't use the Messaging API channel. The documentation for this module is now available. For more information about the module, see [Module](/docs/partner-docs/module/).

2022/01/05

## Sending messages using IFA discontinued

As announced on [December 1, 2021](#partner-news-20211201), sending messages using the IFA (Identifier for Advertisers) of the audience match API is scheduled to be discontinued at the end of December 2021.
...
The LINE Bot Development Guidelines, which were previously provided as PDF files, will now be provided as documentation on the LINE Developers site. In addition, the name of the guidelines has been changed to [Development guidelines for corporate customers](/docs/partner-docs/development-guidelines/). Refer to this documentation from now on.
...
In the API reference for "[Get statistics per aggregation unit](/docs/messaging-api/unit-based-statistics-aggregation/)", there was a mistake in the description of the request body. We've already corrected the mistake but apologize for any inconvenience caused by this error.

See this table for the differences before and after the correction:

- **Assign a unit name to any aggregation unit when sending messages**| Items                                            | Incorrect | Correct |
| ------------------------------------------------ | --------- | ------- |
...
::admonition{title="Updated on May 25, 2021" type="note"}
...
The subject of the [Notification email](/docs/partner-docs/error-notification/#mail) will be changed as follows. We'll also change some of the text in the body of the mail to make it easier to understand.
...
For more information, see [Error notification](/docs/partner-docs/error-notification/) under the options for corporate customers documentation.
...
::admonition
---
title: When is the feature for getting statistics per aggregation unit useful?
type: tip
---
When sending a narrowcast or broadcast message to multiple users, you can specify a request ID to [get user interaction statistics](/reference/messaging-api/#get-message-event) of that particular message.

![User interaction statistics](/media/news/old_statistics_en.png){className="[\"w-fix-520\"]"}

When sending a message to fewer than 20 users, you can't obtain statistics because users' privacy must be protected. However, with the newly-released [Get statistics per aggregation unit](/docs/messaging-api/unit-based-statistics-aggregation/) feature, even if you send a message to a small number of users, by assigning a unit name before sending the message, you can obtain per-unit statistics on multiple messages.

![Aggregating statistics per unit](/media/news/new_statistics_en.png){className="[\"w-fix-520\"]"}
::

For more information, see [Get statistics per aggregation unit](/docs/messaging-api/unit-based-statistics-aggregation/) under the options for corporate customers documentation.
...
For more information, see "[You can now change the icon and display name of your LINE Official Account (2020/3/17)](/news/2020/03/17/icon-nickname-switch/)".
