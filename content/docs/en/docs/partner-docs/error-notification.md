...
__hash__: HiRg-0H1h3vwe-x5fAFlvZ0RF7lU7bYlgPVJ3iZ-XUo
seo:
  title: Error notification
  description: null
---
...
If the user adds your LINE Official Account as a friend or sends a message to your LINE Official Account, LINE Platform sends a webhook event to the URL (bot server) specified in the [LINE Developers Console](/console/) "Webhook URL".
...
- [When the LINE Platform detected an error](#detected-error)
- [When the LINE Platform stopped webhook redelivery](#webhook-redelivery-stopped) (only if Webhook redelivery is enabled)
...
| Main text     | LINE Platform sent a webhook, but your bot server did not respond or returned an error.   Check the reason and details for the error and your bot server's configuration. Then make any necessary changes so that it can receive webhooks properly. |
| Error details | The reason for the error and the date and time of occurrence will be described per situation. For details, see [Email content](#content).                                                                                                           |

::admonition
---
title: You can also check error information in the LINE Developers Console
type: tip
---
You can also check the error information you received in the notification email in the [LINE Developers Console](/console/). For more information, see ["Webhook errors" tab in the LINE Developers Console](#line-developers-console).
::

#### When the LINE Platform stopped webhook redelivery

If you've [enabled Webhook redelivery](/docs/messaging-api/receiving-messages/#enable-webhook-redelivery) in the Messaging API channel settings, the LINE Platform will redeliver the webhook that your bot server failed to receive.
...
| Main text     | The LINE Platform tried to send the webhook for the event(s), but stopped redelivery due to no response from your bot server.   Please visit the LINE Developers site for details. |
| Error details | The reason for the error and the date and time of occurrence will be described per situation. For details, see [Email content](#content).                                          |

For more information about the webhook redelivery, see [Redeliver a webhook that failed to be received](/docs/messaging-api/receiving-messages/#webhook-redelivery).
...
| Reason for error | Overview of reason for error. For more information, see [Check the reason for errors](/docs/messaging-api/check-webhook-error-statistics/#check-error-reason) in the Messaging API documentation.                                                         |
| Detail for error | Details on reason for error. For more information, see [Check the detail for errors](/docs/messaging-api/check-webhook-error-statistics/#check-detail-for-error) in the Messaging API documentation.                                                      |
...
Suppose you received the same error notification content as in the [Notification email sample](#sample-mail). Since the reason for error is `error_status_code` and detail for error is `500`, so [check the reason for errors](/docs/messaging-api/check-webhook-error-statistics/#check-error-reason), you can assume that the bot server responded to the webhook request with the HTTP status code `500`.
...
::admonition{title="On error investigation" type="note"}
...
You can also check the error information received in the notification email on the **Webhook errors** tab of the Messaging API channel on the [LINE Developers Console](/console/).
