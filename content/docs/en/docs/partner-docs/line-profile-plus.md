...
type: note
---
Only corporate users in Japan who have submitted the required applications can use the functions described in this document. To use the information registered with LINE Profile+ via LINE Login, LIFF App, or LINE MINI App, contact your sales representative or [our Sales partners](https://www.lycbiz.com/jp/partner/sales/){rel="[\"nofollow\"]"}. For LINE MINI Apps, this feature is only available for verified MINI Apps.
::

LINE Profile+ is a service for managing the profile information of LINE users. The information that users register with LINE Profile+ is different from usual :glossary-tooltip[[profile information](/glossary/#profile-information)]{glossary-id="profile-information"} and can only be obtained by corporate users who have undergone the application process.

:toc## Differences between profile information and LINE Profile+

For more information on the differences between LINE profile information and LINE Profile+, see [Get user profile information](/docs/basics/user-profile/) in the LINE Platform basics.

- [What is user profile information](/docs/basics/user-profile/#what-is-profile)
- [LINE Profile+](/docs/basics/user-profile/#what-is-line-profile-plus)
...
| Step                             | [Via LIFF App or LINE MINI App](#liff-mini)                                   | [Via LINE Login](#line-login)                                                                            |
| -------------------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 1. Specify scopes                | [Specify scopes on LINE Developers Console](#liff-specify-scope)              | [Specify scopes for authorization URL](#line-login-specify-scope)                                        |
| 2. Get ID token payload          | [Get ID token payload via liff.getDecodedIDToken()](#liff-get-id-token)       | [Validate ID token obtained when issuing access token to get ID token payload](#line-login-get-id-token) |
| 3. Get LINE Profile+ Information | [Get LINE Profile+ information from ID token payload](#liff-get-profile-plus) | [Get LINE Profile+ information from ID token payload](#line-login-get-profile-plus)                      |

### Via LIFF App or LINE MINI App

To get information registered with LINE Profile+ via LIFF App or LINE MINI App, set the [scopes](#scope) of information you want to obtain on the [LINE Developers Console](/console/), then get ID token payload, and you will be able to obtain the LINE Profile+ information of currently logged-in users.

#### 1. Specify scopes on LINE Developers Console

Specify the scopes of information you want to obtain in advance. Select the target channel on the [LINE Developers Console](/console/), and from the **Scope** section under the **Web app settings** tab of the LINE MINI App chanel or the **LIFF** tab of the LINE Login channel, check the scope(s) you would like to use.

For more information on the scopes you can get via LINE Profile+, see [LINE Profile+ scopes](#scope).

![profile plus scope](/media/partner-docs/profile_plus_scopes_en.png){className="[\"border\",\"w-fix-680\"]"}

::admonition{title="Specify openid at the same time" type="note"}
...
When you execute the [`liff.getDecodedIDToken()`](/reference/liff/#get-decoded-id-token) method of the LIFF SDK, you can get a payload of decoded ID token that contains LINE Profile+ information of currently logged-in users of the LIFF App or the LINE MINI App.
...
For more information on LINE Profile+ information included in ID token, see [LINE Profile+ information included in ID token](#id-token).

### Via LINE Login

You can obtain information registered with LINE Profile+ by integrating LINE Login v2.1 into your web app and using an [ID token](/docs/line-login/verify-id-token/#id-tokens).

This page only contains additional information on using LINE Profile+. For more details about how to integrate LINE Login v2.1, see [Integrating LINE Login with your web app](/docs/line-login/integrate-line-login/).
...
For more information about the operation after the user accesses the authorization URL, see [Authentication process](/docs/line-login/integrate-line-login/#authentication-process).

#### 2. Validate ID token obtained when issuing access token to get ID token payload

Information registered with LINE Profile+ is included in the [ID token](/docs/line-login/verify-id-token/#id-tokens). The ID token is included in the response when [issuing the access token](/docs/line-login/integrate-line-login/#get-access-token).
...
ID tokens obtained via [Issue access token](/reference/line-login/#issue-access-token) are encoded in Base64 format (e.g. `eyJhbGciOiJIUzI1NiJ9...`). You can get ID token payload decoded in JSON format by executing [Verify ID token](/reference/line-login/#verify-id-token).
