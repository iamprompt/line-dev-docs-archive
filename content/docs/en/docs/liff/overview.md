...
:markdown-controlsLINE Front-end Framework (LIFF) is a platform for web apps provided by LY Corporation. The web apps running on this platform are called LIFF apps.
...
For more information on functions added to LIFF v2, see the [Release Notes](/docs/liff/release-notes/).

::admonition{title="Try out LIFF features on the web" type="tip"}
LY Corporation provides a web application (LIFF app) for developers called [LIFF Playground](https://liff-playground.netlify.app/){rel="[\"nofollow\"]"}. LIFF Playground allows you to try out basic LIFF features on the web. [The source code for LIFF Playground](https://github.com/line/liff-playground){rel="[\"nofollow\"]"} is available on GitHub.
::

::admonition{title="LIFF apps not compatible with OpenChat" type="note"}
...
| ------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| iOS     | Latest version. [WKWebView](https://developer.apple.com/documentation/webkit/wkwebview){rel="[\"nofollow\"]"} is used.           | In accordance with the recommended system specifications for LINE. * |
| Android | Latest version. [Android WebView](https://developer.android.com/reference/android/webkit/WebView){rel="[\"nofollow\"]"} is used. | In accordance with the recommended system specifications for LINE. * |
| LINE    | Latest version                                                                                                                   | In accordance with the recommended system specifications for LINE. * |

::admonition
---
title: We recommend using the latest versions of OS and LINE for LIFF apps
type: note
---
...
* For more information on the recommended system specifications for LINE, see [Recommended system specifications for LINE](https://help.line.me/line/ios/pc?lang=en&contentId=10002433){rel="[\"nofollow\"]"} in Help Center.
...
The LIFF browser uses [WKWebView](https://developer.apple.com/documentation/webkit/wkwebview){rel="[\"nofollow\"]"} in iOS, and [Android WebView](https://developer.android.com/reference/android/webkit/WebView){rel="[\"nofollow\"]"} in Android. As such, the specifications and behavior of the LIFF browser will also be in accordance with these schemes.

The LIFF browser doesn't support some of the web technologies supported by external browsers. For more information, see [The differences between the LIFF browser and external browser](/docs/liff/differences-between-liff-browser-and-external-browser/).

## LIFF browser cache

[WKWebView](https://developer.apple.com/documentation/webkit/wkwebview){rel="[\"nofollow\"]"} and [Android WebView](https://developer.android.com/reference/android/webkit/WebView){rel="[\"nofollow\"]"}, which are used by the LIFF browser, may save and use displayed content as a cache as instructed by HTTP headers, such as [Cache-Control](https://developer.mozilla.org/ja/docs/Web/HTTP/Reference/Headers/Cache-Control){rel="[\"nofollow\"]"}.

Control caching in the LIFF browser, using HTTP headers such as [Cache-Control](https://developer.mozilla.org/ja/docs/Web/HTTP/Reference/Headers/Cache-Control){rel="[\"nofollow\"]"}.

::admonition{title="On deleting cache" type="note"}
...
![View size](/media/liff/overview/viewTypes.png){className="[\"border\"]"}

Set the view size when you add the LIFF app to your LINE Login channel. For more information, see [Adding a LIFF app to your channel](/docs/liff/registering-liff-apps/#registering-liff-app).
...
When the user taps the action button, either the [multi-tab view](#multi-tab-view) or the [options](#multi-tab-view-option) will be displayed depending on the LINE app version. In LINE version 15.12.0 or later, the multi-tab will be displayed, and in LINE version earlier than 15.12.0, the options will be displayed.

## Multi-tab view

The multi-tab view displays options for the LIFF app currently in use and recently used services.

1. [Options](#multi-tab-view-option)
2. [Recently used services](#multi-tab-view-recent-service)

![](/media/liff/overview/liff-multi-tab-view-en.png){className="[\"w-fix-320\",\"border\"]"}
...
| **Minimize browser**   | Minimizes LIFF browser. For more information, see [Minimizing LIFF browser](/docs/liff/minimizing-liff-browser/).                                                                                                                      |
...
::admonition{title="Permanent link sharing may fail" type="note"}
...
If you use the [`liff.sendMessages()`](/reference/liff/#send-messages) method in a LIFF app that has been reloaded from the recently used services section, an error will occur. For this reason, you can't use the `liff.sendMessages()` method when a LIFF app is reloaded.
...
When developing web apps using LIFF, follow these [LIFF app development guidelines](/docs/liff/development-guidelines/).
...
| ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [LIFF starter app](/docs/liff/trying-liff-app/)                               | This is a starter app for those new to learning about LIFF. The LIFF starter app is only a demo of the LIFF app initialization to help you understand how to start developing LIFF apps. It's recommended for those who want to build something that works first and get a rough idea of what LIFF is all about.                                                                                                                                                                                 |
| [Create LIFF App](/docs/liff/cli-tool-create-liff-app/)                       | This CLI tool allows you to build a development environment for LIFF apps with a single command. Like [Create React App](https://github.com/facebook/create-react-app){rel="[\"nofollow\"]"} by React or [Create Next App](https://nextjs.org/docs/pages/api-reference/cli/create-next-app){rel="[\"nofollow\"]"} by Next.js, by answering questions from Create LIFF App, a development environment including the LIFF app template for you is generated and development can begin immediately. |
| [LIFF CLI](/docs/liff/liff-cli/)                                              | A CLI tool to help you develop LIFF apps more smoothly. The LIFF CLI allows you to do the following:  - Create, update, list, and delete LIFF apps - Create a LIFF app development environment - Debug your LIFF app with the [LIFF Inspector](/docs/liff/liff-plugin/#liff-inspector) - Launch a local develpment server with HTTPS  The [LIFF Mock](/docs/liff/liff-plugin/#liff-mock) feature will be added in a future update.                                                               |
| [LIFF Playground](https://liff-playground.netlify.app/){rel="[\"nofollow\"]"} | You can try out LIFF's features online. [The source code for LIFF Playground](https://github.com/line/liff-playground){rel="[\"nofollow\"]"} is available on GitHub, so developers can set their LIFF ID and deploy their own LIFF Playground on the server.                                                                                                                                                                                                                                     |

## Workflow

To enable the use of the LIFF app by the end user, follow these steps:

1. [Create a channel](/docs/liff/getting-started/) to add your LIFF app to.
2. [Try the LIFF starter app](/docs/liff/trying-liff-app/), or [develop a LIFF app](/docs/liff/developing-liff-apps/).
