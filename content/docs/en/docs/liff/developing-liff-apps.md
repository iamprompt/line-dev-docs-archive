...
__hash__: _6aXm0O3uOg-UEz-fiPKmH2VTMXfS1V257mW4_8jPIM
seo:
  title: Developing a LIFF app
  description: ''
---
...
:markdown-controlsA LIFF app is a web app based on HTML and JavaScript. Here, we'll explain the process of developing a LIFF app and processes specific to building LIFF apps.

:toc## Setting the title of the LIFF app
...
::admonition{title="Manage your SDK version" type="note"}
It is the developer's responsibility to use an appropriate SDK version. To keep your SDK version up to date, check the [LIFF release notes](/docs/liff/release-notes/) on a regular basis, and update your local SDK frequently. For more information about LIFF's versioning policy, see [LIFF SDK (sdk.js) update policy](/docs/liff/versioning-policy/#update-policy).
::

::admonition
---
title: >-
  An error will occur during the build when using the npm version of LIFF
  v2.16.0 or earlier in a project using webpack v5.
type: note
---
[Node.js polyfill has been removed from webpack v5.](https://webpack.js.org/blog/2020-10-10-webpack-5-release/#automatic-nodejs-polyfills-removed){rel="[\"nofollow\"]"} Accordingly, if you use the npm version of LIFF v2.16.0 or earlier in a project using webpack v5, you will get a build error. For more information, see the news from October 26, 2021, [LIFF v2.16.1 released](/news/2021/10/26/release-liff-2-16-1/).
::

To install the LIFF SDK via npm and import it into your app, follow these steps:

1. Run this command in your terminal to install the LIFF SDK via npm:```bash
$ npm install --save @line/liff
```

  
Alternatively, you can run this command in your terminal to install the LIFF SDK via Yarn:```bash
$ yarn add @line/liff
```
2. Import the SDK into your app  
Include the following code in your JavaScript or TypeScript file:```js
...
Type definitions for TypeScript are already included in the `@line/liff` package.::admonition{title="Don't declare or modify window.liff" type="note"}
...
Related page: [https://www.npmjs.com/package/@line/liff](https://www.npmjs.com/package/@line/liff){rel="[\"nofollow\"]"}

::admonition{title="Reducing the LIFF SDK file size" type="tip"}
Using the pluggable SDK reduces the LIFF SDK file size. For more information, see [Pluggable SDK](/docs/liff/pluggable-sdk/).
::

## Initializing the LIFF app

The [`liff.init()`](/reference/liff/#initialize-liff-app) method initializes the LIFF app and enables you to call the other methods of the LIFF SDK from the LIFF app.
...
The process from when the user accesses the :glossary-tooltip[[LIFF URL](/glossary/#liff-url)]{glossary-id="liff-url"} on the LINE app to when the LIFF app is initialized is as follows:

:interactive-svg{src="/media/liff/initializing-liff-app-flow-liff-browser-en.svg" border}For more information, see [LIFF Behaviors from accessing the LIFF URL to opening the LIFF app](/docs/liff/opening-liff-app/#redirect-flow).
...
- `liff.referrer`: Indicates the URL before a LIFF-to-LIFF transition. For more information, see [Get URL from before LIFF-to-LIFF transition](/docs/liff/opening-liff-app/#using-liff-referrer).
...
---
title: >-
  Enabling automatic executing of the liff.login() method when the LIFF app is
  initialized in external browsers
type: tip
---
...
`liffId` needs a specified LIFF app ID, which you can get by adding the LIFF app to your channel. For more information, see [Adding a LIFF app to your channel](/docs/liff/registering-liff-apps/).
...
Call the `liff.login()` method to perform the login process for both the :glossary-tooltip[[LINE's in-app browser](/glossary/#line-iab)]{glossary-id="line-iab"} and :glossary-tooltip[[external browsers](/glossary/#external-browser)]{glossary-id="external-browser"}.
...
---
title: >-
  If you've specified true in the withLoginOnExternalBrowser property when
  executing the liff.init() method
type: tip
---
If you've specified `true` in the `withLoginOnExternalBrowser` property of the `liff.init()` method, the `liff.login()` method will be automatically executed when you initialize your LIFF app, even in external browsers. For more information, see [liff.init()](/reference/liff/#initialize-liff-app) in the LIFF API reference.
::
...
For more information, see [liff.openWindow()](/reference/liff/#open-window) in the LIFF API reference.
...
For more information, see [liff.scanCodeV2()](/reference/liff/#scan-code-v2) in the LIFF API reference.
...
::admonition
---
title: The operation environment of the liff.scanCode2() method
type: note
---
...
To get the email addresses of users, select the `email` scope when [adding a LIFF app to your channel](/docs/liff/registering-liff-apps/). You will get the email address once the user grants permission. The scope selections can be changed in the LIFF tab of the [LINE Developers Console](/console/) even after adding the LIFF app.
::

#### Send user data to the server

When the LIFF app sends user data to the server, it sends the access token or ID token obtained via this method. For more information, see [Using user data in LIFF apps and servers](/docs/liff/using-user-profile/) in the LIFF documentation.

- Execute the `liff.getAccessToken()` method to get the access token of the current user. For more information, see [liff.getAccessToken()](/reference/liff/#get-access-token) in the LIFF API reference.```javascript
// get access token
...
```
- Execute the `liff.getIDToken()` method to get the raw ID token of the current user. For more information, see [liff.getIDToken()](/reference/liff/#get-id-token) in the LIFF API reference.```javascript
liff.init(() => {
...
::admonition{title="Don't send user data to the server" type="warning"}
...
Learn more on how to [Add a LINE Official Account as a friend when logged in (add friend option)](/docs/line-login/link-a-bot/) in the LINE Login documentation.
...
For more information, see [liff.sendMessages()](/reference/liff/#send-messages) in the LIFF API reference.
...
1. In the [LINE Developers Console](/console/), select the LINE Login channel to add the LIFF app.
...
These are the OGP tags supported by LIFF. For more information on OGP tags, see [The Open Graph protocol](https://ogp.me/){rel="[\"nofollow\"]"}.
...
::admonition
---
title: Transitioning up to a higher level than the LIFF endpoint URL
type: note
---
...
The behavior when a LIFF app opened in the LIFF browser is closed by the user or via the [`liff.closeWindow()`](/reference/liff/#close-window) method varies depending on the LINE app version and the LIFF app settings.

### If the LINE app version is 15.12.0 or later

If the LINE app version is 15.12.0 or later, the behavior when closing the LIFF app will change depending on whether the LIFF app meets the [conditions for appearing in the "recently used services" section](/docs/liff/overview/#multi-tab-view-condition) in the multi-tab view.
...
For more information, see [Recently used services](/docs/liff/overview/#multi-tab-view-recent-service).
...
1. [Add the LIFF app to your channel.](/docs/liff/registering-liff-apps/)
2. [Open the LIFF app](/docs/liff/opening-liff-app/)
