...
:markdown-controlsThe channel access token is an opaque string that is used to verify that the application attempting to use the :glossary-tooltip[[channel](/glossary/#channel)]{glossary-id="channel"} has permission to use the channel. With a channel access token, you can use many useful features offered by :glossary-tooltip[[LINE Platform](/glossary/#line-platform)]{glossary-id="line-platform"}, such as the Messaging API.
...
![Channel](/media/basics/channel.png){className="[\"w-fix-480\",\"bg-border\"]"}
...
![Channel access token](/media/basics/channel-access-token.png){className="[\"w-fix-560\",\"bg-border\"]"}

::admonition
---
title: Revoke any channel access tokens suspected of being compromised
type: note
---
...
::admonition
---
title: Can be used repeatedly within the validity period
type: tip
---
...
Up to 30 channel access tokens v2.1 can be issued per channel. Any attempt to issue more than the number that can be issued will result in the issuance request being denied. For more information about channel access tokens v2.1, see [Issue channel access tokens v2.1](/docs/messaging-api/generate-json-web-token/) in the Messaging API documentation.
...
For more information on issuing short-lived channel access tokens, see [Issue short-lived channel access token](/reference/messaging-api/#issue-shortlived-channel-access-token) in the Messaging API reference.

### Long-lived channel access token

Long-lived channel access tokens are channel access tokens that don't expire and can only be issued through the Messaging API channel. This can be issued at any time from the **Messaging API** tab in the Messaging API channel of the [LINE Developers Console](/console/). You can revoke these tokens at any time.
...
![Example of channel access token operation](/media/basics/operate-channel-access-token.png){className="[\"w-fix-560\",\"bg-border\"]"}
...
- [Revoke short-lived or long-lived channel access token](/reference/messaging-api/#revoke-longlived-or-shortlived-channel-access-token)
