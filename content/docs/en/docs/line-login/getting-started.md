...
__hash__: L4c87F7H2JinzLxjVS3pcdUbaRfHOb69C40CWtOwsrw
seo:
  title: Getting started with LINE Login
  description: null
---
...
:markdown-controlsThis page explains how to get started with LINE Login by deploying a simple starter web app. The web app lets users log in with their LINE accounts. You can use the access token issued when a user logs in to get the user's profile.
...
::admonition{title="iOS/Android/Unity starter apps" type="tip"}
We also offer starter apps for specific platforms:

- [Trying the starter app - iOS Swift](/docs/line-login-sdks/ios-sdk/swift/try-line-login/)
- [Trying the sample app - Android](/docs/line-login-sdks/android-sdk/try-line-login/)
- [Trying the starter app - Unity](/docs/line-login-sdks/unity-sdk/try-line-login/)
::
...
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LINE account       | An account for the LINE app. You need a LINE account to try out the starter app. To create a LINE account, [download](https://line.me/){rel="[\"nofollow\"]"} LINE for iOS or LINE for Android and sign up. For more information about creating a new LINE account, see [Create a new account](https://guide.line.me/ja/signup/line-signup.html){rel="[\"nofollow\"]"} (only available in Japanese) in the LINE user's guide.                                                                                                                                                                                                                |
| Provider           | Provider is a concept that describes the person or organization that provides the application. Create a provider on the [LINE Developers Console](/console/). A LINE user has a different user ID for each provider.                                                                                                                                                                                                                                                                                                                                                                                                                         |
| LINE Login channel | A channel forms the connection between your app and the LINE Platform. Create a channel within the provider. You need to create a channel for each app. Create a LINE Login channel on the [LINE Developers Console](/console/register/line-login/channel/).    Notes: - If you've never logged in to the LINE Developers Console, you'll be asked to register as a developer first.   - The steps for creating your LINE Login channel are explained in [Step 1: Create your LINE Login channel](#step-1-create-channel). - When creating a LINE Login channel to use a starter app, makes sure you select **Web app** under **App types**. |
| Heroku account     | [Heroku](https://www.heroku.com/){rel="[\"nofollow\"]"} is a service that offers hosting for web apps. Deploying the starter app to Heroku means you don't need your own server.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Heroku CLI         | The [Heroku Command Line Interface (CLI)](https://devcenter.heroku.com/articles/heroku-cli){rel="[\"nofollow\"]"} is necessary for using certain Heroku functions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

::admonition{title="Heroku's free plan has been discontinued" type="note"}
Heroku's free plan has been discontinued as of November 27, 2022. If you would like to try this starter app for free, use another platform. For more information, see [Heroku’s Next Chapter](https://www.heroku.com/blog/next-chapter/){rel="[\"nofollow\"]"}.
::
...
A [channel](/docs/line-developers-console/overview/#channel) is the conduit through which your app connects to the LINE Platform. Create a LINE Login channel in the [LINE Developers Console](/console/register/line-login/channel/) for each of your web apps.
...
| **Region to provide the service**        | ✅               | The region you want to provide your LINE Login service. One of:    - Japan - Thailand - Taiwan - Indonesia  *If you want to provide your service in multiple regions, create a channel for each region. | -                                                               |
...
| **App types**                            | ✅               | The type of app you plan to integrate LINE Login with. One of:    - Web app - Mobile app  *In the example of deploying a starter app, select **Web app**.                                               | -                                                               |
| **Email address**                        | ✅               | Email address to receive important updates about the channel                                                                                                                                            | -                                                               |
| **Privacy policy URL**                   | See description | The URL of the app's privacy policy. Required if your provider is a [certified provider](/docs/line-developers-console/overview/#certified-provider).                                                   | Permission consent screen when launching LINE Login or LIFF App |
| **Terms of use URL**                     | ❌               | The URL of the app's terms of use                                                                                                                                                                       | Permission consent screen when launching LINE Login or LIFF App |
| **LINE Developers Agreement**            | ✅               | Read and agree to the [LINE Developers Agreement](https://terms2.line.me/LINE_Developers_Agreement?lang=en){rel="[\"nofollow\"]"}.                                                                      | -                                                               |
...
1. Go to the [line-login-starter](https://github.com/line/line-login-starter){rel="[\"nofollow\"]"} repository on GitHub.
2. In the [README](https://github.com/line/line-login-starter){rel="[\"nofollow\"]"}, click **Deploy to Heroku**.
3. Fill in the required information on the "Create New App" page in Heroku:

  - Heroku app name   
  Must be unique. Suggestion: `line-login-starter-{YYYYMMDD}`{v-pre=""}
  - Region
  - Config Variables
...
1. From the [LINE Developers Console](/console/), select the channel you created in Step 1.
...
![LINE Login starter app login](/media/line-login/getting-started/line-login-starter-app-login.png){className="[\"border\",\"w-fix-320\"]"}
2. Click **Log in**.  
You'll be redirected to our standard login page. The URL starts with `https://access.line.me/oauth2/v2.1/` and contains a number of query parameters. To learn what they mean, read [Integrating LINE Login with your web app](/docs/line-login/integrate-line-login/).
...
Check your app's logs with [Heroku CLI](https://devcenter.heroku.com/articles/heroku-cli){rel="[\"nofollow\"]"}.

1. Log in to Heroku from the command line.```sh
$ heroku login
```
2. Check the logs.```sh
...
1. Clone the [line-login-starter](https://github.com/line/line-login-starter){rel="[\"nofollow\"]"} GitHub repository.```sh
git clone https://github.com/line/line-login-starter.git
```
2. `cd` into `line-login-starter`.
3. Add a remote for Heroku to your local repository.```sh
$ heroku git:remote -a {Heroku app name}
```
4. Make edits and commit changes (optional).```sh
$ git add .
$ git commit -m "First commit"
```
5. Push changes to Heroku master.```sh
$ git push heroku master
```

## Step 6: Publish your channel (optional)

LINE Login channels are created with "Developing" status. With this status, only users with the Admin or Tester role (see [Managing roles](/docs/line-developers-console/managing-roles/)) can use the LINE Login channel. If you want to allow other users to access your app, you have to change the status of the app to "Published". To do so, open your LINE Login channel on the [LINE Developers Console](/console/), and click the **Developing** status at the top of the page.
...
For more information on linking a Business ID with a LINE account, see [Link your Business ID with your LINE account](/docs/line-developers-console/login-account/#link-business-account-with-line-account) in the LINE Developers Console documentation.
