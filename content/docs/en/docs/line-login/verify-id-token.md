...
:markdown-controlsThe LINE Platform issues ID tokens compliant with the [OpenID Connect](https://openid.net/developers/how-connect-works/){rel="[\"nofollow\"]"} specification, allowing you to securely obtain user :glossary-tooltip[[profile information](/glossary/#profile-information)]{glossary-id="profile-information"} (user ID, display name, profile picture, email address) from the LINE Platform.

If you have LINE Profile+ permission, you can also safely obtain data registered with :glossary-tooltip[[LINE Profile+](/glossary/#line-profile-plus)]{glossary-id="line-profile-plus"} (name, gender, birthday, phone number, address). For more information, see [Get user data registered with LINE Profile+](/docs/partner-docs/line-profile-plus/#getting-profile-plus).

:toc## Get an ID token

You can also get an ID token when you [get an access token](/docs/line-login/integrate-line-login/#get-access-token).

::admonition
---
title: You can also get an ID token with the LIFF app.
type: tip
---
You can also use [liff.getIDToken()](/reference/liff/#get-id-token) to get an ID token.
::

## About ID tokens

ID tokens are JSON web tokens (JWT) with information about the user. The ID token consists of a [header](#header), [payload](#payload), and [signature](#signature) separated by period (.) characters. Each part is a base64url-encoded value. For more information, see the [JWT](https://datatracker.ietf.org/doc/html/rfc7519){rel="[\"nofollow\"]"} specification.
...
To validate the ID token, write a verification code or use the [Verify ID token](/reference/line-login/#verify-id-token) endpoint. For more information about using the endpoint to verify the ID token, see [Get profile information from an ID token](#get-profile-info-from-id-token).
...
| `kid`    | String | Public key ID. Included in a header only when the value of `alg` is `ES256`. For more information on the `kid` property, see the [JSON Web Key (JWK) document](https://datatracker.ietf.org/doc/html/rfc7517#section-4.5){rel="[\"nofollow\"]"}. |
...
The user's information is found in the payload section. You can only get the main profile information. You can't get the user's :glossary-tooltip[[subprofile](/glossary/#subprofile)]{glossary-id="subprofile"}.
...
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `ES256` (ECDSA using P-256 and SHA-256) | Element in the [JSON Web Key (JWK) document URL](https://api.line.me/oauth2/v2.1/certs){rel="[\"nofollow\"]"} that contains the `kid` property in the header |
| `HS256` (HMAC using SHA-256)            | ::glossary-tooltip{glossary-id="channel-secret"}   [Channel secret](/glossary/#channel-secret)   ::                                                          |

For more information on ID token verification, see [ID Token Validation](https://openid.net/specs/openid-connect-core-1_0.html#IDTokenValidation){rel="[\"nofollow\"]"} on OpenID Connect Core 1.0.

For information about the OpenID provider, see [OpenID Provider Configuration Document](https://access.line.me/.well-known/openid-configuration){rel="[\"nofollow\"]"}.

## Get profile information from an ID token

When using the information contained in the ID token, write a verification code or use the [Verify ID token](/reference/line-login/#verify-id-token) endpoint on LINE Login to verify the ID token.
...
For more information, see [Verify ID token](/reference/line-login/#verify-id-token) in the LINE Login API reference.
