---
title: Set up in-app purchase
navigation: true
description: null
meta: '{"tags":null,"author":null,"last_updated":null,"source_language":"en"}'
path: /en/docs/line-mini-app/in-app-purchase/iap-settings
__hash__: Nt6sH5sCHM_icHxJEVvWDIf5X1mO-l93DSvhoDwR4Gk
seo:
  title: Set up in-app purchase
  description: null
---
...
:markdown-controlsThis page explains the items to configure after your [application to use in-app purchase](/docs/line-mini-app/in-app-purchase/request-iap-review/) has been approved.

In the **In-app purchase** tab within the LINE MINI App channel, you can [register the webhook URL](#register-webhook-url) and [register testers](#register-testers) for the test payment.

![](/media/line-mini-app/in-app-purchase/iap-settings-tab-en.png){className="[\"border\"]"}

::admonition
---
title: If the In-app purchase settings tab doesn't appear
type: tip
---
...
2. In the **Webhook URL for developing** input field, enter the URL of the server that will receive notifications. The URL must start with `https://`.
...
1. In the LINE Developers Console, select the **In-app purchase** tab and then select the **In-app purchase settings** tab.
2. In the **Webhook URL for published** input field, enter the URL of the server that will receive notifications. The URL must start with `https://`.
3. Click the **Update** button.
...
For more information on the actual testing method, see the [Test payment guide](/docs/line-mini-app/in-app-purchase/implement-in-app-purchase/#test-payment-guide).
...
2. Select the **In-app purchase** tab and click **In-app purchase settings**.
3. From the **Select a tester** dropdown list in the "Tester permission for the test payment feature" section, select the account you wish to register. The dropdown list displays accounts that have already been added in the **Roles** tab of the LINE MINI App channel.
