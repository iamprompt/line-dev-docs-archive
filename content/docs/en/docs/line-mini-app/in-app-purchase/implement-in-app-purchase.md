...
__hash__: GrYFMZ2hCnh7LmwN-Mm2O2Y1D0O40hKswajBAwOwK5Q
seo:
  title: Integrate the in-app purchase feature
  description: null
---
...
:markdown-controlsThis page explains how to integrate the in-app purchase feature into your LINE MINI App.
...
- Your [application to use in-app purchase feature](/docs/line-mini-app/in-app-purchase/request-iap-review/) has been approved.
- A server for your LINE MINI App is available.
- A webhook endpoint (webhook URL) is available. (*)

* Register the webhook URL on the [LINE Developers Console](/console/) after your application to use in-app purchase has been approved. For more information on how to register, see [Register the webhook URL](/docs/line-mini-app/in-app-purchase/iap-settings/#register-webhook-url).

## Implementation flow

Integrate in-app purchase with the following flow:

1. [Check if your environment is compatible with in-app purchase](#check-the-environment)
2. [Get information about purchasable items](#get-item-information)
3. [Obtain user consent for in-app purchase](#get-user-consent)
4. [Reserve a purchase from your LINE MINI App server](#reserve-payment)
5. [Start the purchase transaction at the store](#start-transaction)
6. [Receive webhook and process purchase completion](#receive-webhook)

### 1. Check if your environment is compatible with in-app purchase

Call the [`liff.isApiAvailable()`](/reference/liff/#is-api-available) method to determine if your environment supports in-app purchase.
...
Even if you confirm that the environment supports in-app purchase using the `liff.isApiAvailable()` method, in-app purchase can't be used if you can't obtain user consent in "[3. Obtain user consent for in-app purchase](#get-user-consent)" or if the consent is revoked later.
...
Among pre-defined items, items supported by your LINE MINI App can be determined according to the policy of the service provider using in-app purchase. Call the [`liff.iap.getPlatformProducts()`](/reference/line-mini-app/#get-platform-products) method to get localized prices, currencies, and item names for those items.
...
Use the [`liff.iap.requestConsentAgreement()`](/reference/line-mini-app/#request-consent-agreement) method to obtain user consent for "[Terms of Use: LINE In-App Purchase System](https://terms.line.me/line_iap_tou_1?lang=en){rel="[\"nofollow\"]"}".
...
Before starting the purchase transaction at the app store (App Store, Google Play), reserve the purchase from your LINE MINI App server using the "[Reserve purchase](/reference/line-mini-app/#reserve-purchase)" endpoint.
...
After completing the purchase reservation, start the [app store payment](/reference/line-mini-app/#create-payment) from your LINE MINI App.
...
When the purchase transaction succeeds, the LINE Platform verifies that the payment was made correctly by checking with the store. When the payment is verified to be correct, the LINE Platform notifies the webhook endpoint of the purchase completion webhook event. For more information on the webhook event that will be notified, see [Purchase complete event](/reference/line-mini-app/#purchase-complete-event) in the LINE MINI App API reference.
...
When you receive the [purchase complete](/reference/line-mini-app/#purchase-complete-event) webhook event notified by the LINE Platform, check the contents on your LINE MINI App server and grant the item to the user. The specific implementation of how to grant items to users depends on the implementation of the LINE MINI App being developed.
...
Each user purchase is assigned a unique `orderId` issued at the time of [purchase reservation](#reserve-payment). Determine whether the transaction has already been processed using the `orderId`. Additionally, grant an item only once for each purchase.

Always determine purchase completion based on the webhook event.

#### Verify the webhook signature

To prevent forged requests, verify the signature using the `x-line-signature` request header when receiving webhooks. For more information, see [Verify the webhook signature](/docs/line-mini-app/in-app-purchase/iap-guidelines/#verify-webhook-signature).
...
You can use the "[Get webhook event history](/reference/line-mini-app/#webhook-events-history)" endpoint to retrieve the history of webhook events that were previously sent.

If you need to perform recovery for webhook events that fail to be received, use this endpoint.

For more information, see [Get webhook event history](/reference/line-mini-app/#webhook-events-history) in the LINE MINI App API reference.

## Test payment guide

After integrating the in-app purchase feature into your LINE MINI App, test payments become available in the LINE MINI App channel for Developing. With test payments, you can verify a series of operations such as purchasing items and [checking purchase history](/docs/line-mini-app/in-app-purchase/implement-in-app-purchase/#purchase-history) on a LINE MINI App.
...
- Have [Admin or Tester roles for the relevant LINE MINI App channel](#channel-permission)
- Have [tester permissions for the test payment feature](#tester-permission)

### LINE MINI App channel roles

To use the test payment feature for LINE MINI Apps, you need the Admin role or the Tester role for the LINE MINI App channel. Set permissions in the **Role settings** tab on the [LINE Developers Console](/console/).

For more information on how to set roles, see [Adding developers, editing roles, and deleting developers on channel](/docs/line-developers-console/managing-roles/#role-settings-for-channel) in the LINE Developers Console documentation.

Note that only developers with the Admin role for a channel can add or edit channel roles. For more information on the differences in roles, see [LINE MINI App channel](/docs/line-developers-console/managing-roles/#roles-for-channel-line-mini-app) in the LINE Developers Console documentation.
...
For more information on how to set permissions, see [Register testers](/docs/line-mini-app/in-app-purchase/iap-settings/#register-testers).
...
1. [Register a tester](/docs/line-mini-app/in-app-purchase/iap-settings/#register-testers) in the relevant LINE MINI App channel.
2. Share the LIFF URL of the LINE MINI App for Developing with the tester. The LIFF URL can be confirmed in the **Web app settings** tab on the LINE Developers Console.
3. The tester launches the LINE MINI App from the specified LIFF URL and performs a payment.
...
In the "In-App Purchases" screen shown below, the values in the red-bordered section reflect the actual prices and currency at the time of [applying to use in-app purchase](/docs/line-mini-app/in-app-purchase/request-iap-review/), at the time of [purchase reservation request](#reserve-payment), or when the user purchases from the store.

![](/media/line-mini-app/in-app-purchase/purchase-history-en.png){className="[\"w-fix-400\"]"}
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | The item name (product name) is displayed.                                                                                                                                                                                                                                                                                                                                                                | When you [reserve purchase](/reference/line-mini-app/#reserve-purchase), specify it in `shopProductName` of the [request body](/reference/line-mini-app/#reserve-purchase-request-body).   * Set an appropriate value so that the item purchased by the user can be identified.             |
| 2   | The name of our service providing the in-app purchase feature (LINE MINI App) and the service provider's service name are displayed.      **Display patterns:**- Device language set to Japanese: `LINEミニアプリ・{Service provider's service name}` - Other languages: `LINE MINI App・{Service provider's service name}`  * Multi-language support for the provider's service name isn't currently available. | The service provider's service name is set in the LINE MINI App channel's **Basic settings** tab > **Channel name**.   * If you change the channel name of a verified LINE MINI App, you need to request a [re-review](/docs/line-mini-app/service/update-service/).                        |
| 3   | The app store used for payment (App Store or Google Play), and the product ID corresponding to the price are displayed.                                                                                                                                                                                                                                                                                   | The product IDs are pre-registered by us in the app store, and developers can't edit them. You can check the combinations of product IDs and prices in the [List of product IDs for items available for purchase via in-app purchase](/docs/line-mini-app/in-app-purchase/iap-product-id/). |
