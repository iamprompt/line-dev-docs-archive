...
:markdown-controlsThis page explains the specification constraints, design considerations, and recommended implementation methods for using the in-app purchase feature in LINE MINI Apps.

When using the in-app purchase feature, follow the development guidelines below. Also, be sure to refer to the [LINE MINI App development guidelines](/docs/line-mini-app/development-guidelines/).

**Prohibited matters**

- [Don't restrict access by IP address](#prohibit-ip-address-restriction)

**Required matters**

- [Verify access token validity](#verify-access-token)
...
- [Don't send duplicate payment notifications](#payment-notifications)

## Prohibited matters

### Don't restrict access by IP address

On servers that receive webhooks, don't restrict access based on the IP address of the LINE Platform sending webhook requests. The IP address of the LINE Platform isn't disclosed and may change without notice. Instead of access control by IP address, use [signature verification](#verify-webhook-signature) to deny requests from unauthorized sources.

## Required matters

### Verify access token validity

When making a purchase reservation, use the [Verify access token validity](/reference/line-login/#verify-access-token) endpoint to verify the access token validity, channel ID, and access token validity period on the server side of your LINE MINI App.
...
For more information on webhook signature verification, see [Verify the Webhook signature](/docs/messaging-api/verify-webhook-signature/) in the Messaging API documentation.
