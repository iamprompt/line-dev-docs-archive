...
To use Common Profile Quick-fill, your LINE MINI App must be verified and you must apply to use Quick-fill. For more information, see [Steps for using Quick-fill](#process).
::
...
- [Step 1. Prepare a verified MINI App](#process-step-1)
- [Step 2. Apply to use Quick-fill and develop](#process-step-2)

### Step 1. Prepare a verified MINI App

Quick-fill is only available in verified MINI Apps. Therefore, you must first prepare a verified MINI App to integrate Quick-fill. For more information, see [Process from LINE MINI App development to release](/docs/line-mini-app/quickstart/#overall-process).
...
- [Step 2-1. Apply for Quick-fill and obtain approval](#process-step-2-1)
- [Step 2-2. Specify the Quick-fill scope in the LINE Developers Console](#process-step-2-2)
- [Step 2-3. Integrate Quick-fill](#process-step-2-3)
- [Step 2-4. Request review of the LINE MINI App](#process-step-2-4)
...
- [[Single application] Usage application form (Excel file)](https://workers-hub.ent.box.com/s/06w8vzqxfwx2e031oq2q9ztj7ca8p7h8){rel="[\"nofollow\"]"} (Only available in Japanese)
- [[Multiple applications] Usage application form (Excel file)](https://workers-hub.ent.box.com/s/xrwjm892d1uxsiblptfgoj07r0v5zwbp){rel="[\"nofollow\"]"} (Only available in Japanese)
...
[Application form](https://form-business.yahoo.co.jp/claris/enqueteForm?inquiry_type=miniapp-quick-fill){rel="[\"nofollow\"]" className="[\"btn-in-docs\",\"custom\"]"} (Only available in Japanese)

#### Step 2-2. Specify the scope of Quick-fill in the LINE Developers Console

After your application has been accepted to use Quick-fill, specify the scope of the information you want to use. In the [LINE Developers Console](/console/), select the target LINE MINI App channel, and in the **Scope** section of the **Web app settings** tab, check the box for the scope you want to use.
...
---
title: >-
  Behavior when Quick-fill and Channel consent simplification are both enabled
  at the same time
type: tip
---
If you enable both Quick-fill and [Channel consent simplification](/docs/line-mini-app/develop/channel-consent-simplification/) at the same time, users won't be able to disable the toggle button for the Common Profile on the Verification screen. We plan to make a fix for this behavior in the future. For more information about the Verification screen, see [Request permissions other than the `openid` scope on the verification screen](/docs/line-mini-app/develop/channel-consent-simplification/#request-permissions-other-than-openid).
::

#### Step 2-3. Integrate Quick-fill

After specifying the scope, integrate Quick-fill into your LINE MINI App. For more information on development, see [Integrate Quick-fill with the LIFF Plugin](#use-liff-plugin).

When developing a LINE MINI App that integrates Quick-fill, follow the instructions below:

- [Common Profile Quick-fill design regulations](/docs/line-mini-app/quick-fill/design-regulations/)
- [LINE MINI App development guidelines](/docs/line-mini-app/development-guidelines/)
- [LIFF app development guidelines](/docs/liff/development-guidelines/)
...
To develop Quick-fill, you need to use the LIFF SDK and the [LIFF plugin](/docs/liff/liff-plugin/). For information about the LIFF SDK version that the LIFF plugin works with, see [LIFF SDK version](#liff-sdk-version).

You can integrate the LIFF SDK into your LINE MINI App in one of two ways:

- [Specify a CDN path to integrate Quick-fill](#specify-cdn-path)
- [Use the npm package to integrate Quick-fill](#use-npm-package)

Once you've integrated the LIFF SDK into your LINE MINI App, you can enable the Quick-fill LIFF plugin by passing the Quick-fill LIFF plugin to the [`liff.use()`](/reference/liff/#use) method as shown below:
...
- [`liff.$commonProfile.get()`](/reference/line-mini-app/#get-common-profile)
- [`liff.$commonProfile.getDummy()`](/reference/line-mini-app/#get-dummy-common-profile)
- [`liff.$commonProfile.fill()`](/reference/line-mini-app/#fill-common-profile)
...
For more information, see [Specify the CDN path](/docs/liff/developing-liff-apps/#specify-cdn-path) in the LIFF documentation.
...
For more information, see [Use the npm package](/docs/liff/developing-liff-apps/#use-npm-package) in the LIFF documentation.
...
- [LIFF SDK version](#liff-sdk-version)
- [Node.js version](#nodejs-version)

LINE MINI Apps use the [LINE Front-end Framework (LIFF)](/docs/liff/overview/). For more information about the recommended environment for Quick-fill, see the [Recommended operating environment](/docs/liff/overview/#operating-environment) section of the LIFF documentation.

::admonition
---
title: Transition destinations where the LIFF app is guaranteed to work
type: note
---
...
Use LIFF SDK v2.19.0 or later, as the LIFF plugin is used in the development of Quick-fill. For more information on LIFF plugin, see [LIFF plugin](/docs/liff/liff-plugin/) in the LIFF documentation.
...
For more information on integrating the LIFF SDK into your LIFF app, see [Integrating the LIFF SDK with the LIFF app](/docs/liff/developing-liff-apps/#integrating-sdk) in the LIFF documentation.
...
If these scopes don't display on the LINE Developers Console, see [Step 2-1. Apply for Quick-fill and obtain approval](#process-step-2-1).

Users cannot select and allow some scopes on the [channel consent screen](/docs/line-mini-app/develop/configure-console/#consent-screen-settings). They can either allow or disallow these scopes as "Management Information (Common Profile) in the Account Center" in bulk.

## The `scopes` parameters that can be specified and its return value

The `scopes` parameters that can be specified for [`liff.$commonProfile.get()`](/reference/line-mini-app/#get-common-profile) and [`liff.$commonProfile.getDummy()`](/reference/line-mini-app/#get-dummy-common-profile) and the return values for each are as follows:
...
| 5      | `sex-enum`         | Gender                 | number    | 1 (fixed length)         | N/A                              | - `0`: Male - `1`: Female - `2`: Other - `3`: No answer |
...
You can use [`liff.$commonProfile.getDummy()`](/reference/line-mini-app/#get-dummy-common-profile) to get dummy data for Common Profile. You can specify the dummy data to get with `caseId` from the 10 types provided.

| `caseId` | `family-name`                                                                                        | `given-name`                                                                                         | `family-name-kana`                                                                                   | `given-name-kana`                                                                            | `sex-enum` | `bday-day` | `bday-month` | `bday-year` | `tel`                                                                                                                                                                                                    | `email`                                                                                                                                                                                                                                                                                                                                                                                                                     | `postal-code`                                          | `address-level1`                                                               | `address-level2`                                      | `address-level3`                                                                                                                             | ::div   ---   className:     - min-w-300px   ---   `address-level4`   ::                    |
| -------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| 7        | ﾀﾞﾐｰﾀ                                                                                                | ﾀﾞﾐｵ                                                                                                 | ダミータ                                                                                                 | ダミーオ                                                                                         | 2          |            | 3            |             | 09001234567090012345670900123456709001234567090012345670900123456709001234567090012345670900123456709001234567090012345670900123456709001234567090012345670900123456709001234567090012345670900123456709 | [dummy_39@yahoo.co.jp](mailto:dummy_39@yahoo.co.jp)                                                                                                                                                                                                                                                                                                                                                                         | 1020094                                                |                                                                                |                                                       |                                                                                                                                              |                                                                                             |
| 8        | ダミ！？                                                                                                 | ダミ夫@                                                                                                 | ダミータ                                                                                                 | ダミーオ                                                                                         | 3          | 12         |              | 1998        | 09001234567                                                                                                                                                                                              | [dummy_39@yahoo.co.jp](mailto:dummy_39@yahoo.co.jp)                                                                                                                                                                                                                                                                                                                                                                         | 1020094                                                | 🍀                                                                             | 🍀🍀                                                  | 🍀🍀🍀                                                                                                                                       | 🍀🍀🍀🍀                                                                                    |
| 9        | 🐶🐶🐶                                                                                               | ダミ💚                                                                                                 | ダミータ                                                                                                 | ダミーオ                                                                                         | 0          | 12         | 3            | 1998        |                                                                                                                                                                                                          | [dummy_39@yahoo.co.jp](mailto:dummy_39@yahoo.co.jp)                                                                                                                                                                                                                                                                                                                                                                         | 102-0094                                               | 東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都東京都 | 千代田区千代田区千代田区千代田区千代田区千代田区千代田区千代田区千代田区千代田区千代田区千代田区千代田区千 | 紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2紀尾井町1-2 | 東京ガーデンテラス紀尾井町東京ガーデンテラス紀尾井町東京ガーデンテラス紀尾井町東京ガーデンテラス紀尾井町東京ガーデンテラス紀尾井町東京ガーデンテラス紀尾井町東京ガーデンテラス紀尾井町 |
| 10       | ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田ダミー田 | ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫ダミー夫 | ダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータダミータ | ダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオダミーオ | 1          | 12         | 3            | 1998        | 09001234567                                                                                                                                                                                              | [dummy_39@yahoo.co.jp](mailto:dummy_39@yahoo.co.jp)                                                                                                                                                                                                                                                                                                                                                                         | N5X 1N7                                                |                                                                                | 千代田区                                                  | 紀尾井町1-2                                                                                                                                      | 東京ガーデンテラス紀尾井町                                                                               |

## Options for getting Common Profile information

When getting Common Profile information using [`liff.$commonProfile.get()`](/reference/line-mini-app/#get-common-profile), you can specify the following options for each scope. These options are all set to `true` by default, so specify `false` to disable them.
...
| `excludeNonJp`  | true          | Whether to exclude phone numbers with 12 or more digits. If `true`, an empty string and error information are returned when the phone number has 12 or more digits.             | - `tel`                        |
| `digitsOnly`    | true          | Whether to exclude postal codes that aren't numbers. If `true`, an empty string and error information are returned when the postal code contains characters other than numbers. | - `postal-code`                |

## API reference

For more information about the client API used for Quick-fill, see [Common Profile Quick-fill](/reference/line-mini-app/#quick-fill) in the LINE MINI App API reference.
