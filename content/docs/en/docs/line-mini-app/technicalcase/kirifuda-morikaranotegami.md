...
title: >-
  ...
  Forest launched on the LINE MINI App
navigation: true
description: ''
meta: >-
...
:markdown-controls:partial{content="documentation-partials/api-use-case/announcement-of-articles/"}![KIRIFUDA Inc.](/media/line-mini-app/technicalcase/kirifuda-morikaranotegami/en/kirifuda-morikaranotegami-logo.png){className="[\"w-fix-240\"]"}
...
### Reason for using LINE MINI App

Normally, receiving NFTs requires installing a dedicated wallet or creating an account, which presents a high barrier to Web3 adoption. KIRIFUDA utilized the LINE API and LINE Front-end Framework (LIFF) to build a system where wallet generation and NFT-related setup are completed just by launching the LINE MINI App.

The client's representative highly evaluated the solution, stating, "The ability to receive NFTs simply by launching LINE, without any complex procedures, was a key advantage". Because the system is linked to LINE IDs, it eliminates the need for additional account registration. Furthermore, the ability to consistently deliver messages to specific users also creates strong synergy.

Soon after launch, the visitor feedback was positive, with many saying, "Receiving an NFT so effortlessly feels like a completely new experience", and "The appealing stamp designs really motivated me to collect them all". Users realized how their everyday eco-friendly actions were seamlessly translated into meaningful environmental contributions. The project was also featured on a TV Tokyo program, drawing attention for its innovative blend of environmental action and digital experience. Despite the remote forest location, connectivity issues were minimal, and field operations ran smoothly, requiring only QR codes and basic guidance. This resulted in high praise for the overall user experience (UX).
