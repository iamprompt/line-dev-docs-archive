...
navigation: true
description: >-
  Basic structure and precautions of using LINE Developers Console for LINE MINI
  App.
...
seo:
  title: LINE Developers Console Guide for LINE MINI App
  description: >-
    Basic structure and precautions of using LINE Developers Console for LINE
    MINI App.
---

# :page-title

:markdown-controlsUnderstand the basic structure and precautions of the [LINE Developers Console](/console/) before requesting a review.

:toc## LINE Developers Console for LINE MINI App

LINE Developers Console is a tool for developing and testing your LINE MINI App, as well as for submitting LINE MINI Apps for the verification review to make them verified MINI Apps. LINE Developers Console for LINE MINI App can be used by anyone who is a permitted customer in the [LINE MINI App Policy](https://terms2.line.me/LINE_MINI_App?lang=en){rel="[\"nofollow\"]"}.
...
| ---------------- | ---------------------------------------------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| **Developing**   | Internal channel for developing and testing                            | Always "Developing"       | Only administrators who have accepted the permissions you granted   - You can check the settings on the LINE MINI App channel settings screen on the LINE Developers Console                                                        | Only testers who have accepted the permissions you granted |
| **Review**       | Internal channel that LY Corporation uses to review your LINE MINI App | "Developing" at all times | - Administrators who have accepted the permissions you granted   - You can check the settings on the LINE MINI App channel settings screen on the LINE Developers Console - LY Corporation reviewers                                | Only LY Corporation reviewers                              |
| **Published**    | Internal channel published to users                                    | "Published" at all times  | Only administrators who have accepted the permissions you granted   - You can check the information about the "Published" channel by clicking the **Published Data** button in the upper right corner of the LINE MINI App channel. | End users                                                  |

::admonition{title="You can't change the channel status" type="note"}
You can't change the status of the internal channels.
::

::admonition{title="LINE MINI App Tester enrollment" type="tip"}
In order to add a user to test the LINE MINI App, enroll them as a tester of the LINE MINI App channel. For more information, see [Managing roles](/docs/line-developers-console/managing-roles/).
::
...
You can specify the URL with basic authentication in the **Endpoint URL** for **Developing** or **Review**. For more information, see [Use basic authentication to restrict access to your LINE MINI App before it is released](/docs/line-mini-app/develop/develop-overview/#use-basic-authentication).

::admonition
---
title: Each internal channel has a different LIFF ID
type: note
---
- When you call the [`liff.init()`](/reference/liff/#initialize-liff-app) method on the LINE MINI App, specify a different LIFF ID for each internal channel. For example, when you execute initialization from the "Review" channel, use `liff.init()` to specify the LIFF ID of the Review channel before initializing it. If you can't launch a LINE MINI App on all the internal channels, check whether the following two LIFF IDs match.
...
---
title: >-
  Differences between the [LIFF] tab of the LINE Login channel and the [Web app
  settings] tab of the LINE MINI App channel
type: note
---
...
::admonition{title="Settings on the LINE Developers Console" type="tip"}
...
Use a [stateless channel access token](/docs/basics/channel-access-token/#stateless-channel-access-token) for the LINE MINI App channel.
...
![Channel ID and Channel Secret](/media/line-mini-app/channel_id_secret.png){className="[\"border\",\"w-fix-800\"]"}

::admonition
---
title: Channel access token must be issued for each internal channel
type: note
---
...
---
title: >-
  You can't edit the Company or owner's country or region for the existing LINE
  MINI App channels
type: note
---
...
| LINE MINI App              | Header subtext [(reference)](/docs/line-mini-app/discover/ui-components/#header)                                                                       |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
