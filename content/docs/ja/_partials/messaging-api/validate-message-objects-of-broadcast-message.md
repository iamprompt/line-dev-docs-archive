---
title: Validate Message Objects Of Broadcast Message
navigation: true
description: ''
meta: '{}'
...
:api-endpoint{protocol="post" endpoint="https://api.line.me/v2/bot/message/validate/broadcast"}::reference-with-code
  :::reference-content
  [メッセージオブジェクト](#message-objects)の配列が、[ブロードキャストメッセージを送る](#send-broadcast-message)エンドポイントの[リクエストボディ](#send-broadcast-request-body)の`messages`プロパティの値として有効かを検証します。`messages`プロパティ以外のプロパティの値が有効かは検証しません。
  :::
