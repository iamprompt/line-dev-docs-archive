---
title: LINEプラットフォームで発生した問題の報告方法を教えてください。
navigation: true
description: >-
  LINEプラットフォームで発生した問題について報告したり、改善要望を伝えたりしたい場合は、ヘルプセンターの「問題報告フォーム」からお問い合わせください。お問い合わせのすべてにご返答できない場合がありますので、あらかじめご了承ください。
meta: >-
  {"date":"2021-06-15 00:00 UTC","tags":"contact, line-platform, line-login,
  liff, line-mini-app, messaging-api","priority":1000,"locale":"ja"}
path: /ja/_partials/faq/how-do-i-report-an-issue
__hash__: 4xdR7f05kqX0oPJ1UqG1qL_9E2SG3guGoIVNoZ6pHCw
seo:
  title: LINEプラットフォームで発生した問題の報告方法を教えてください。
  description: >-
    LINEプラットフォームで発生した問題について報告したり、改善要望を伝えたりしたい場合は、ヘルプセンターの「問題報告フォーム」からお問い合わせください。お問い合わせのすべてにご返答できない場合がありますので、あらかじめご了承ください。
---

LINEプラットフォームで発生した問題について報告したり、改善要望を伝えたりしたい場合は、ヘルプセンターの「[問題報告フォーム](https://contact-cc.line.me/serviceId/11556){rel="[\"nofollow\"]"}」からお問い合わせください。お問い合わせのすべてにご返答できない場合がありますので、あらかじめご了承ください。

テクニカルサポートを必要とされている場合は、FAQの「[LINE APIに関する問い合わせがしたいです。テクニカルサポートの窓口はどこですか？](/faq/#i-need-technical-support)」を参照してください。
