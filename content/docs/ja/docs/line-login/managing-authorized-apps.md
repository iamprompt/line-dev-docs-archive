...
:markdown-controlsユーザーがLINEログインチャネルを利用するときは、:glossary-tooltip[[ユーザーID](/glossary/#user-id)]{glossary-id="user-id"}の取得などに同意する必要があります。同意後、ユーザーはいつでも同意内容を確認したり、同意を取り消したりできます。
...
| 対象     | 説明                                                                                                                                                          |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ユーザー   | - 同意を取り消したアプリにLINEログインしようとすると、再び同意画面が表示されます。 - 同意するまでLINEログインできません。                                                                                         |
| プロバイダー | - 取得済みのアクセストークンを利用しても、ユーザーIDやプロフィール情報を取得できません。 - リフレッシュトークンを利用できないため、アクセストークンを更新できません。 - ユーザーが再び同意してLINEログインするまで、アクセストークンは取得できないため、ユーザーIDやプロフィール情報を取得できません。 |

::admonition{title="ユーザーが同意を取り消したことを尊重してください" type="note"}
...
[LINEユーザーデータポリシー](https://terms2.line.me/LINE_Developers_user_data_policy?lang=ja){rel="[\"nofollow\"]"}に従い、ユーザー情報は正しく取り扱ってください。LINEユーザーデータポリシーに従わない場合、サービス停止等の措置を取らせていただきます。
::
