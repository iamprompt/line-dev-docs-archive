...
:markdown-controls:glossary-tooltip[[LINE公式アカウント](/glossary/#line-official-account)]{glossary-id="line-official-account"}がユーザーの:glossary-tooltip[[プロフィール情報](/glossary/#profile-information)]{glossary-id="profile-information"}を取得するには、ユーザーがプロフィール情報の取得に同意している必要があります。
...
なおユーザーがプロフィール情報の取得に同意していない場合、以下のWebhookイベントオブジェクトや、エンドポイントからのレスポンスに、そのユーザーのプロフィール情報は含まれません。また、Webhookの[メンバーシップイベント](/reference/messaging-api/#membership-event)は送信されません。

- [Webhookイベントオブジェクト](/reference/messaging-api/#webhook-event-objects)の[送信元ユーザー](/reference/messaging-api/#source-user)
- [テキストメッセージオブジェクト](/reference/messaging-api/#wh-text)の`mention`オブジェクト
- [LINE公式アカウントを友だち追加したユーザーのリストを取得する](/reference/messaging-api/#get-follower-ids)エンドポイント
- [メンバーシップに加入しているユーザーの一覧を取得する](/reference/messaging-api/#get-membership-user-ids)エンドポイント
- [グループトークのメンバーのユーザーIDを取得する](/reference/messaging-api/#get-group-member-user-ids)エンドポイント
- [複数人トークのメンバーのユーザーIDを取得する](/reference/messaging-api/#get-room-member-user-ids)エンドポイント

::admonition{title="ユーザーのプロフィール情報が取得できない" type="tip"}
