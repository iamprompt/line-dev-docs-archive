...
:markdown-controlsこのトピックでは、ビーコン端末に組み込むSecure message生成機能の開発中に利用できる、サンプルのデータとコードについて説明します。Secure message生成のアルゴリズムについては、「[Secure messageを生成する](/docs/messaging-api/beacon-device-spec/#generating-secure-message)」を参照してください。
