...
__hash__: ly6ornKgBuUyenxUZc8TKnvtmFGIv_58GWtki3-2SoA
seo:
  title: 送信したメッセージの統計情報を取得する
  description: null
---
...
:markdown-controlsMessaging APIでは、LINE公式アカウントから送信したメッセージを、ユーザーがどのように操作したかを統計情報として取得できます。統計情報の取得方法は、送信するメッセージの種類によって異なります。

- [すべての友だち、またはオーディエンスや属性で絞り込んだ友だちに送信したメッセージの統計情報](#get-statistics-on-narrowcast-or-broadcast)
  - ブロードキャストメッセージ
  - ナローキャストメッセージ
- [特定の友だちやグループトーク、または電話番号を指定して送信したメッセージの統計情報](#get-statistics-on-push-multicast-or-line-notification-messages)
  - プッシュメッセージ
  - マルチキャストメッセージ
  - LINE通知メッセージ
...
[ナローキャストメッセージ](/reference/messaging-api/#send-narrowcast-message)または[ブロードキャストメッセージ](/reference/messaging-api/#send-broadcast-message)では、ユーザーがメッセージをどのように操作したかを、送信リクエストごとの統計情報として取得できます。詳しくは、『Messaging APIリファレンス』の「[ユーザーの操作に基づく統計情報を取得する](/reference/messaging-api/#get-message-event)」を参照してください。

## 特定の友だちやグループトーク、または電話番号を指定して送信したメッセージの統計情報

[プッシュメッセージ](/reference/messaging-api/#send-push-message)、[マルチキャストメッセージ](/reference/messaging-api/#send-multicast-message)、[LINE通知メッセージ](/docs/partner-docs/line-notification-messages/overview/)では、ユーザーがメッセージをどのように操作したかを、ユニットごとの統計情報として取得できます。
...
::admonition{title="LINE通知メッセージでユニット名を指定する" type="tip"}
Messaging APIのプッシュメッセージとマルチキャストメッセージだけでなく、法人ユーザー向けオプションであるLINE通知メッセージでも、メッセージの送信時にユニット名を指定できます。詳しくは、『LINE通知メッセージドキュメント』の「[LINE通知メッセージの統計情報を取得する](/docs/partner-docs/line-notification-messages/statistics/)」を参照してください。
::

下図のように、ユニット名を付与してメッセージを送信することで、ユニットごとのメッセージの統計情報を取得できるようになります。

![](/media/news/customAggregationUnits_ja.png){className="[\"w-fix-920\"]"}
...
「[ユニットごとの統計情報を取得する](/reference/messaging-api/#get-statistics-per-unit)」エンドポイントのレスポンスには、送信通数および送信対象となったユーザー数は含まれません。また、`overview.uniqueImpression`は、集計対象期間中にメッセージを1回以上開封したユニークユーザー数です。そのため、このレスポンスだけでは、送信通数を分母とする開封率や、送信対象のユーザー数を分母とするリーチ率を算出できません。

### ユニット名を付与する

統計情報を取得するには、プッシュメッセージ、マルチキャストメッセージ、またはLINE通知メッセージを送信する際に、任意の集計単位のユニット名を付与する必要があります。ユニット名を付与するには、メッセージ送信時にリクエストボディの`customAggregationUnits`プロパティにユニット名を指定してください。メッセージ送信時に指定できるユニット名は1つだけです。プッシュメッセージやマルチキャストメッセージの送信方法について詳しくは、『Messaging APIリファレンス』の「[メッセージ](/reference/messaging-api/#messages)」を参照してください。
...
::admonition{title="ユニット名の後付けや変更について" type="tip"}
...
::admonition{title="ユニット名の制限について" type="tip"}
...
ユニット名を付与して送信したメッセージの統計情報は、「[ユニットごとの統計情報を取得する](/reference/messaging-api/#get-statistics-per-unit)」エンドポイントで取得できます。以下の例では、`promotion_a`という名前のユニットの統計情報を取得しています。
...
なお、当月中に付与したユニット名は、「[当月に付与したユニット名のリストを取得する](/reference/messaging-api/#get-a-list-of-unit-names-assigned-during-this-month)」エンドポイントで取得できます。前月以前に付与したユニット名を確認するためのエンドポイントはありません。
...
![](/media/messaging-api/insight/new-item-message-example-ja.png){className="[\"w-fix-360\",\"border\"]"}
