...
__hash__: g9V-wCFL1MyW5zorjyZLgjVI5s-qZynTSwIDXrPefdE
seo:
  title: モジュールチャネルからMessaging APIを利用する
  description: null
---
...
- [モジュールチャネルからLINE公式アカウントの情報を取得する](/docs/partner-docs/module-technical-using-messaging-api/#get-line-oa-info-from-module-channel)
- [注意事項](#notes)
...
LINEマーケットプレイスで提供するモジュールチャネルでは、ユーザー個別の識別子である:glossary-tooltip[[ユーザーID](/glossary/#user-id)]{glossary-id="user-id"}は、Lからはじまる68桁の文字列で構成されます。
...
- [短期のチャネルアクセストークン](/docs/basics/channel-access-token/#short-lived-channel-access-token)
...
チャネルアクセストークンを発行する際に必要な情報は、[LINE Developersコンソール](/console/)のモジュールチャネルの［**チャネル基本設定**］タブで確認できます。

::admonition{title="長期のチャネルアクセストークンは使用できません" type="note"}
...
スコープは、モジュールチャネルをアタッチするときに指定し、LINE公式アカウントの管理者から利用の許可を得る必要があります。詳しくは、「[LINE公式アカウントの管理者に認可を要求する](/docs/partner-docs/module-technical-attach-channel/#request-auth-from-line-oa-admin)」を参照してください。
...
::parameter-table
  ...
  ボットのユーザーIDは、「[モジュールチャネルの提供者の操作で連携（アタッチ）する](/reference/partner-docs/#link-attach-by-operation-module-channel-provider)」のレスポンスや[Attachedイベント](/reference/partner-docs/#attached-event)で取得できます。
...
以下は、モジュールチャネルからMessaging APIで[プッシュメッセージ](/reference/messaging-api/#send-push-message)を送信する場合の例です。
...
各エンドポイントのレート制限について詳しくは、『Messaging APIリファレンス』の「[レート制限](/reference/messaging-api/#rate-limits)」を参照してください。

### モジュールチャネルから送信したメッセージの統計情報を取得する

複数のユーザーに送信した[プッシュメッセージ](/reference/messaging-api/#send-push-message)や[マルチキャストメッセージ](/reference/messaging-api/#send-multicast-message)について、ユーザーがメッセージをどのように操作したかを示す統計情報を、ユニットごとに集計できます。
...
また、当月中（その月の1日から末日）に付与したユニット名の種類数も同様に、LINE公式アカウントのボットとユニット名の組み合わせで集計されます。
...
スコープについて詳しくは、「[LINE公式アカウントの管理者に認可を要求する](/docs/partner-docs/module-technical-attach-channel/#request-auth-from-line-oa-admin)」を参照してください。
::
...
| `mode`プロパティの値 | 説明                                                                                                                                                                    |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `active`      | Webhookイベントを受信したチャネルは、アクティブです。   このWebhookイベントを受信したWebhook URLサーバーから、返信メッセージやプッシュメッセージなどを送信できます。                                                                      |
| `standby`     | Webhookイベントを受信したチャネルは、待機状態です。   このWebhookイベントを受信したWebhook URLサーバーは、メッセージの送信を控えてください。      なお、待機状態のチャネルに届くWebhookイベントには、`replyToken`プロパティが含まれません。そのため、応答メッセージは利用できません。 |
...
![同じサービスをアタッチ](/media/partner-docs/module-technical/attach-same-service-ja.png){className="[\"border\"]"}
...
| イベントタイプ                                                       | 説明                                                                                                                                                                                        |
| ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Attachedイベント](/reference/partner-docs/#attached-event)       | モジュールチャネルが、LINE公式アカウントにアタッチされたことを示すイベント                                                                                                                                                   |
| [Detachedイベント](/reference/partner-docs/#detached-event)       | モジュールチャネルが、LINE公式アカウントからデタッチされたことを示すイベント                                                                                                                                                  |
| [Activatedイベント](/reference/partner-docs/#activated-event)     | [Acquire Control API](/reference/partner-docs/#acquire-control-api)を呼び出して、モジュールチャネルがActive Channelに切り替わったことを示すイベント                                                                        |
| [Deactivatedイベント](/reference/partner-docs/#deactivated-event) | [Acquire Control API](/reference/partner-docs/#acquire-control-api)または[Release Control API](/reference/partner-docs/#release-control-api)を呼び出して、モジュールチャネルがStandby Channelに切り替わったことを示すイベント |
| [botSuspendイベント](/reference/partner-docs/#botsuspend-event)   | LINE公式アカウントが一時停止状態（Suspend）になったことを示すイベント                                                                                                                                                  |
| [botResumedイベント](/reference/partner-docs/#botresumed-event)   | LINE公式アカウントが一時停止状態（Suspend）から復帰したことを示すイベント                                                                                                                                                |

::admonition{title="主導権（Chat Control）の変化を検知するには" type="tip"}
...
| イベントタイプ                                                                                                                     | 説明                                                                                                                                                                                                                                                          |
| --------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [Activatedイベント](/reference/partner-docs/#activated-event) - [Deactivatedイベント](/reference/partner-docs/#deactivated-event) | LINE公式アカウントにアタッチされているモジュールチャネルが、Acquire Control APIまたはRelease Control APIを呼び出し、チャットの主導権（Chat Control）が切り替わった際に送信されるイベント                                                                                                                                     |
| - [フォローイベント](/reference/messaging-api/#follow-event) - [フォロー解除イベント](/reference/messaging-api/#unfollow-event)               | エンドユーザーがLINE公式アカウントを友だち追加したり、ブロックしたりすると送信されるイベント。   LINE公式アカウントをブロックして再度友だち追加すると、主導権（Chat Control）は自動的にデフォルト状態にリセットされます。「[Default Active](/docs/partner-docs/module-technical-chat-control/#default-active)」の機能が付与されたモジュールチャネルの場合は、自動的にActive Channelになります。 |
::

::admonition{title="LINE公式アカウントの一時停止状態（Suspend）について" type="tip"}
...
モジュールチャネルをアタッチした、複数のLINE公式アカウントのボットの基本情報をリストで取得します。詳しくは、『法人ユーザー向けオプションAPIリファレンス』の「[モジュールをアタッチしたボットのリストを取得する](/reference/partner-docs/#get-multiple-bot-info-api)」を参照してください。
