...
:markdown-controlsLIFFアプリに適切なLIFF SDKを組み込むために、LIFFのバージョニングポリシーを正しく理解してください。
...
LIFFでは、LIFF v2.1.13リリース以降、以下の2種類のCDNパスを用意しています。[LIFFアプリにLIFF SDKを組み込む](/docs/liff/developing-liff-apps/#integrating-sdk)ときに、目的に合ったCDNパスを指定してください。
...
::admonition{title="後方互換性を維持するためのCDNパスの廃止について" type="note"}
