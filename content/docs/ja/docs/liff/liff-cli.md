...
:markdown-controls:toc## LIFF CLIとは
...
- LIFFアプリの開発環境を作成する
- LIFFアプリを[LIFF Inspector](/docs/liff/liff-plugin/#liff-inspector)でデバッグする
- ローカル開発サーバーをHTTPSで起動する

今後のアップデートで[LIFF Mock](/docs/liff/liff-plugin/#liff-mock)の機能も追加される予定です。
...
`channel`コマンドを使うと、LIFF CLIで管理するチャネルを追加したり、デフォルトのチャネルを設定したりできます。なお、チャネルはあらかじめ[LINE Developersコンソール](/console/)で作成する必要があります。
...
| `-e`、`--endpoint-url` | ✅   | LIFFアプリのエンドポイントURLを指定します。LIFFアプリを実装したウェブアプリのURLです（例：`https://example.com`）。LIFF URLを利用してLIFFアプリを起動した際に、このURLが利用されます。  URLスキームは**https**である必要があります。なお、URLフラグメント（#URL-fragment）は指定できません。 |
| `-v`、`--view-type`    | ✅   | LIFFアプリの画面サイズ。以下のいずれかの値を指定します。  - `full` - `tall` - `compact`  詳しくは、「[LIFFブラウザの画面サイズ](/docs/liff/overview/#screen-size)」を参照してください。                                                     |
...
| `--endpoint-url` |     | LIFFアプリのエンドポイントURLを指定します。LIFFアプリを実装したウェブアプリのURLです（例：`https://example.com`）。LIFF URLを利用してLIFFアプリを起動した際に、このURLが利用されます。  URLスキームは**https**である必要があります。なお、URLフラグメント（#URL-fragment）は指定できません。 |
| `--view-type`    |     | LIFFアプリの画面サイズ。以下のいずれかの値を指定します。  - `full` - `tall` - `compact`  詳しくは、「[LIFFブラウザの画面サイズ](/docs/liff/overview/#screen-size)」を参照してください。                                                     |
...
`scaffold`コマンドを使うと、[Create LIFF App](/docs/liff/cli-tool-create-liff-app/)を通じてLIFFアプリのひな形を作成できます。LIFFアプリのプロジェクト名を`scaffold`コマンドに渡すと、そのプロジェクト名を使ってCreate LIFF Appを実行します。

```bash
$ liff-cli scaffold my-app --liff-id 1234567890-AbcdEfgh
```

Create LIFF Appについて詳しくは、「[Create LIFF AppでLIFFアプリの開発環境を構築する](/docs/liff/cli-tool-create-liff-app/)」を参照してください。
...
| `-v`、`--view-type`    | ✅ ※2 | LIFFアプリの画面サイズ。以下のいずれかの値を指定します。  - `full` - `tall` - `compact`  詳しくは、「[LIFFブラウザの画面サイズ](/docs/liff/overview/#screen-size)」を参照してください。                                                |
| `-e`、`--endpoint-url` |      | LIFFアプリのエンドポイントURLを指定します。LIFFアプリのデプロイ先のURLです（例：`https://example.com`）。LIFF URLを利用してLIFFアプリを起動した際に、このURLが利用されます。  URLスキームは**https**である必要があります。なお、URLフラグメント（#URL-fragment）は指定できません。 |
...
`serve`コマンドに`--inspect`オプションを指定すると、LIFFアプリを[LIFF Inspector](/docs/liff/liff-plugin/#liff-inspector)でデバッグできます。
...
LIFF CLIでは、プロキシとして[ngrok](https://ngrok.com/){rel="[\"nofollow\"]"}をサポートしています。ngrokを使用するには、`serve`コマンドの`--proxy-type`オプションに以下のいずれかの値を指定します。
...
`--proxy-type`オプションに`ngrok`を指定すると、ローカルプロキシサーバーの代わりに[ngrok](https://github.com/ngrok/ngrok-javascript){rel="[\"nofollow\"]"}を使うことができます。これにより、ローカルの開発サーバーを外部に公開できます。ngrokを使用する際は、環境変数として`NGROK_AUTHTOKEN`にngrokの認証トークンを設定してください。
...
::admonition{title="ngrok-v1は非推奨です" type="note"}
ngrok v1はすでに開発やメンテナンスを終了しているため、`ngrok-v1`は非推奨です。ngrokを使用する際は、プロキシの種類に[`ngrok`](#serve-proxy-type-ngrok)を指定してください。
::

`--proxy-type`オプションに`ngrok-v1`を指定すると、ローカルプロキシサーバーの代わりに[ngrok v1](https://github.com/inconshreveable/ngrok){rel="[\"nofollow\"]"}を使うことができます。これにより、ローカル開発サーバーを外部に公開できます。なお、この機能を使うには、[ngrok v1](https://github.com/inconshreveable/ngrok){rel="[\"nofollow\"]"}と[node-pty](https://www.npmjs.com/package/node-pty){rel="[\"nofollow\"]"}を別途インストールする必要があります。
...
次の手順に従って、localhostに対して有効な証明書（`localhost.pem`）と秘密鍵（`localhost-key.pem`）を作成してください。ここでは[mkcert](https://github.com/FiloSottile/mkcert){rel="[\"nofollow\"]"}を使います。mkcertについて詳しくは、mkcertの[README](https://github.com/FiloSottile/mkcert/blob/master/README.md){rel="[\"nofollow\"]"}を参照してください。
...
1. `mkcert -install`を実行し、ローカル認証局を作成します。

```bash
$ mkcert -install
```

1. `mkcert localhost`を実行し、localhostに対して有効な証明書（`localhost.pem`）と秘密鍵（`localhost-key.pem`）を作成します。
...
| `--proxy-type`                 |      | 使用するプロキシの種類。以下のいずれかの値を指定します。  - `local-proxy`：ローカルプロキシ - `ngrok`：[ngrok](https://github.com/ngrok/ngrok-javascript){rel="[\"nofollow\"]"} - `ngrok-v1`：[ngrok v1](https://github.com/inconshreveable/ngrok){rel="[\"nofollow\"]"}（非推奨）  デフォルト値は`local-proxy`です。 |
